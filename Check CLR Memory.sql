...
---------------------------------------------------------------------------------------------------------------
SELECT	Type
	,dbaadmin.dbo.dbaudf_FormatBytes(single_pages_kb + multi_pages_kb + virtual_memory_committed_kb,'KB') TotalCLR
	,dbaadmin.dbo.dbaudf_FormatBytes(virtual_memory_reserved_kb,'KB') ReservedCLR
from	sys.dm_os_memory_clerks
where	type Like '%CLR%'
-- FOGLIGHT VERSION
/*
SELECT	Type
	,(single_pages_kb + multi_pages_kb + virtual_memory_committed_kb) TotalCLR
	,(virtual_memory_reserved_kb) ReservedCLR
from	sys.dm_os_memory_clerks
where	type Like '%CLR%'
*/
...
---------------------------------------------------------------------------------------------------------------
select		Type
		,dbaadmin.dbo.dbaudf_FormatBytes(sum(page_size_in_bytes*pages_allocated_count),'BYTES') as SQLCLR_Memory
from		sys.dm_os_memory_objects
where		type like '%CLR%'
GROUP BY	Type;
-- FOGLIGHT VERSION
/*
select		Type Memory_Object_Type
		,sum(page_size_in_bytes*pages_allocated_count) as Total_CLR_Memory
		,COUNT(*) Memory_Object_Count
from		sys.dm_os_memory_objects
where		type like '%CLR%'
GROUP BY	Type;
*/
...
SELECT		CLR.[STATE]
		...
		,OSW.LAST_WAIT_TYPE
		,OSW.is_sick
		,OSW.is_in_cc_exception
		,OSW.is_fatal_exception
		,OSW.is_inside_catch

		,OSW.[STATE] [Worker_State]
		,OST.task_state
		
		,OSW.context_switch_count [Worker_context_switch_count]
		,OST.context_switches_count [Task_context_switches_count]
		
		,OSW.pending_io_count [Worker_pending_io_count]
		,OST.pending_io_count [Task_pending_io_count]
		
		,OSW.pending_io_byte_count [Worker_pending_io_byte_count]
		,OST.pending_io_byte_count [Task_pending_io_byte_count]

		,REQ.session_id
		,REQ.blocking_session_id
		,(SELECT dbaadmin.dbo.dbaudf_ConcatenateUnique(CAST(session_id AS VarChar(10))) FROM SYS.DM_EXEC_REQUESTS WHERE blocking_session_id = REQ.session_id) AS blocked_sessions
		,COALESCE(
			NULLIF(SUBSTRING(SQLT.text
				,CASE WHEN REQ.statement_end_offset > REQ.statement_start_offset THEN REQ.statement_start_offset-1 ELSE 0 END
				,CASE WHEN REQ.statement_end_offset > REQ.statement_start_offset THEN REQ.statement_end_offset-REQ.statement_start_offset ELSE LEN(SQLT.text)-REQ.statement_start_offset END
				),'')
			,SQLT.text
			) Statement_Text

FROM		SYS.DM_CLR_TASKS AS CLR
LEFT JOIN	SYS.DM_OS_WORKERS AS OSW
	ON	OSW.TASK_ADDRESS = CLR.SOS_TASK_ADDRESS
LEFT JOIN	SYS.DM_OS_TASKS OST
	ON	OST.task_address = OSW.task_address
LEFT JOIN	SYS.DM_EXEC_REQUESTS REQ
	ON	REQ.session_id = OST.session_id
	AND	REQ.request_id = OST.request_id
CROSS APPLY	sys.dm_exec_sql_text(REQ.sql_handle) SQLT
...
---------------------------------------------------------------------------------------------------------------
--
--					QUERIES WITING FOR MEMORY GRANTS
--
---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
...
SELECT
   SUM(CONVERT(BIGINT,Size) * Free)/power(1024.,4)  AS [Total avail mem, GB]
  ,CAST(MAX(Size)AS BIGINT)       /power(1024.,4)  AS [Max free size, GB]
 FROM VAS_Summary
 WHERE Free <> 0


-- http://channel9.msdn.com/Events/DevDays/DevDays-2011-Netherlands/Devdays052
-- http://www.toadworld.com/platforms/sql-server/b/weblog/archive/2013/04/28/dmv-9-digging-out-details-of-clr-tasks-sys-dm-clr-tasks.aspx
-- 
-- 
-- http://msdn.microsoft.com/en-us/library/ms187720.aspx
-- http://msdn.microsoft.com/en-us/library/ms186922.aspx
...
INNER JOIN dbaadmin.sys.assemblies AS a
ON l.assembly_id = a.assembly_id
   where a.name = 'GettyImages.Operations.CLRTools';

select appdomain_id, creation_time, db_id, user_id, state
from sys.dm_clr_appdomains a
where appdomain_address = 
(select l.appdomain_address 
