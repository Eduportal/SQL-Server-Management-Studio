...
IF OBJECT_ID('dbo.dbaudf_max_datetime') IS NOT NULL 
	DROP PROCEDURE dbo.dbaudf_max_datetime;
GO
...
IF OBJECT_ID('dbo.dbasp_Diaper_Check') IS NOT NULL 
	DROP PROCEDURE dbo.dbasp_Diaper_Check;
GO

CREATE PROCEDURE dbo.dbasp_Diaper_Check
	...
	@LogToCentral tinyint = 0		/*0= No Logging to Central Server (default). 1= Yes Logging to Central Server. 2= Yes Logging to Central Server with no Table Output */
/*
dbasp_Diaper_Check

For help and how-to info, visit:

	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Dynamic%20Index%20Analysis%20Performance%20Evaluation%20Report.aspx

	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Indexhealth%20AbnormalPsychology.aspx
	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Indexhealth%20AggressiveIndexes.aspx
	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Indexhealth%20IndexFeatures.aspx
	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Indexhealth%20IndexHoarder.aspx
	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Indexhealth%20Indexaphobia.aspx
	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Indexhealth%20SelfLoathing.aspx
	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Indexhealth%20Workaholics.aspx
	https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database%20Wiki/Index%20Analysis/Indexhealth%20DuplicateIndex.aspx

How to use:
--	Diagnose:
		EXEC dbo.dbasp_Diaper_Check @DatabaseName='AdventureWorks';
--	Return detail for a specific table:
		EXEC dbo.dbasp_Diaper_Check @DatabaseName='AdventureWorks', @SchemaName='Person', @TableName='Person';
...
DECLARE @WikiBaseAddress		VarChar(8000)
DECLARE @WikiPage_Index			VarChar(8000)
DECLARE @WikiPage_Abnormal_Psychology	VarChar(8000)
DECLARE @WikiPage_Aggressive_Indexes	VarChar(8000)
DECLARE @WikiPage_Index_Features	VarChar(8000)
DECLARE @WikiPage_Index_Hoarder		VarChar(8000)
DECLARE @WikiPage_Indexaphobia		VarChar(8000)
DECLARE @WikiPage_Self_Loathing		VarChar(8000)
DECLARE @WikiPage_Workaholics		VarChar(8000)
DECLARE @WikiPage_Duplicate_Index	VarChar(8000)
...
SELECT	@SQLServerProductVersion	= CAST(SERVERPROPERTY('ProductVersion') AS NVARCHAR(128))
	,@SQLServerEdition		= CAST(SERVERPROPERTY('EngineEdition') AS INT)			/* We default to online index creates where EngineEdition=3*/
	,@FilterMB			= 250
	,@rundate			= CAST(CONVERT(VarChar(12),getdate(),101)AS DATETIME)
	,@UNCPath			= NULL
	,@target_env			= 'amer'
	,@target_server			= 'SEAPDBASQL01'
	,@target_share			= 'SEAPDBASQL01_dbasql\IndexAnalysis'
	,@retry_limit			= 5
	,@Fill_Factor			= 98
	,@PopulateDMVsForAll		= 1
	,@UNCPath			= COALESCE(@UNCPath,'\\' + LEFT(@@SERVERNAME,(CHARINDEX('\',@@SERVERNAME+'\')-1)) + '\' + REPLACE(@@SERVERNAME,'\','$') + '_dbasql\dba_reports')
	,@object_id			= OBJECT_ID(@DatabaseName+'.'+@SchemaName+'.'+@TableName)
	,@database_id			= db_id(@DatabaseName)
	,@IndexScript			= ''
	,@LocalPath			= @UNCPath
	,@sqlserver_start_time		= (SELECT login_time From sys.sysprocesses WHERE spid = 1)

	,@WikiBaseAddress		= 'https://gettyimages-my.sharepoint.com/personal/steve_ledridge_gettyimages_com/DBCollab/Database Wiki/Index Analysis/'
	,@WikiPage_Index		= 'Dynamic Index Analysis Performance Evaluation Report.aspx'
	,@WikiPage_Abnormal_Psychology	= 'Indexhealth AbnormalPsychology.aspx'
	,@WikiPage_Aggressive_Indexes	= 'Indexhealth AggressiveIndexes.aspx'
	,@WikiPage_Index_Features	= 'Indexhealth IndexFeatures.aspx'
	,@WikiPage_Index_Hoarder	= 'Indexhealth IndexHoarder.aspx'
	,@WikiPage_Indexaphobia		= 'Indexhealth Indexaphobia.aspx'
	,@WikiPage_Self_Loathing	= 'Indexhealth SelfLoathing.aspx'
	,@WikiPage_Workaholics		= 'Indexhealth Workaholics.aspx'
	,@WikiPage_Duplicate_Index	= 'Indexhealth DuplicateIndex.aspx'
...
BEGIN TRY
	BEGIN
		...
		BEGIN
			SET @msg=N'dbasp_Diaper_Check is only supported on SQL Server 2005 and higher. The version of this instance is: ' + @SQLServerProductVersion;
			RAISERROR(@msg,16,1);
		END
		...
		RAISERROR(N'Starting run. dbasp_Diaper_Check(TM) v2.02 - Jan 30, 2014', 0,1) WITH NOWAIT;
		...
		IF OBJECT_ID('tempdb..#IndexHealthCheckResults') IS NOT NULL 
			DROP TABLE #IndexHealthCheckResults;
		
		IF OBJECT_ID('tempdb..#IndexCreateTsql') IS NOT NULL	
			DROP TABLE #IndexCreateTsql;

		RAISERROR (N'Create temp tables.',0,1) WITH NOWAIT;
		CREATE TABLE #IndexHealthCheckResults
			(
			  IHCR_id INT IDENTITY PRIMARY KEY,
		...
		CREATE TABLE #dbtable
			(
			DBName	SYSNAME NULL,
			ParentObject nvarchar(max),
			Object nvarchar(max),
			Field nvarchar(max),
			VALUE nvarchar(max),
			DB_Start_Date DateTime NULL
			)

		exec ('insert #dbtable(ParentObject,Object,Field,Value) 
			exec(''dbcc traceon(3604);dbcc dbtable('''''+@DatabaseName+''''') with tableresults'')')

		exec ('UPDATE #dbtable SET DBName = '''+@DatabaseName+''' WHERE DBName is NULL')

		DELETE		#dbtable 
		WHERE		Field !='m_StartupTick'
		
		UPDATE		#dbtable 
			SET	DB_Start_Date = dateadd(ms,CAST(VALUE AS BIGINT),@sqlserver_start_time) 
		...
		[more_info] AS N'EXEC dbo.dbasp_Diaper_Check @DatabaseName=' + QUOTENAME([database_name],'''') + 
...
BEGIN
	...
	BEGIN  
		SELECT  N'Missing index.' AS Finding ,
				@WikiBaseAddress + @WikiPage_Indexaphobia AS URL ,
...
BEGIN;
	...
	BEGIN;
		...
		RAISERROR('check_id 1: Duplicate keys', 0,1) WITH NOWAIT;
			WITH	duplicate_indexes
					  AS ( SELECT	[object_id], key_column_names
						   ...
						   HAVING	COUNT(*) > 1)
				INSERT	#IndexHealthCheckResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
											   secret_columns, index_usage_summary, index_size_summary )
						SELECT	1 AS check_id, 
								ip.index_sanity_id,
								'Multiple Index Personalities' AS findings_group,
								'Duplicate keys' AS finding,
								@WikiBaseAddress + @WikiPage_Duplicate_Index AS URL,
		...
		INSERT	#IndexHealthCheckResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
				...
				SELECT	11 AS check_id, 
						...
						@WikiBaseAddress + @WikiPage_Aggressive_Indexes AS URL,
			...
			RAISERROR(N'check_id 20: >=7 NC indexes on any given table. Yes, 7 is an arbitrary number.', 0,1) WITH NOWAIT;
				INSERT	#IndexHealthCheckResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
						...
						SELECT	20 AS check_id, 
								...
								@WikiBaseAddress + @WikiPage_Index_Hoarder AS URL,
			...
			IF @number_indexes_with_includes = 0 
				INSERT	#IndexHealthCheckResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
						...
						SELECT	30 AS check_id, 
								...
								N'No indexes use includes' AS finding, @WikiBaseAddress + @WikiPage_Index_Features AS URL,
			...
			IF @percent_indexes_with_includes <= 3 AND @number_indexes_with_includes > 0 
				INSERT	#IndexHealthCheckResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
						...
						SELECT	31 AS check_id,
								...
								@WikiBaseAddress + @WikiPage_Index_Features AS URL,
		...
		SELECT	DISTINCT 
			33 AS check_id, 
				...
				N'Feature-Phobic Indexes' AS findings_group,
				N'Potential filtered index based on column name (is%, %archive%, %active%, %flag%)' AS finding, 
				@WikiBaseAddress + @WikiPage_Index_Features AS URL,
				i.index_name + N' is a candidate for filtering' AS details,
				i.index_definition, 
				i.secret_columns,
				i.index_usage_summary, 
				sz.index_size_summary
		...
		BEGIN
			...
			INSERT	#IndexHealthCheckResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
					...
					SELECT	40 AS check_id, 
							...
							@WikiBaseAddress + @WikiPage_Self_Loathing AS URL,
			...
			INSERT	#IndexHealthCheckResults ( check_id, findings_group, finding, URL, details, index_definition,
							   secret_columns, index_usage_summary, index_size_summary, create_tsql, index_sanity_id )
					SELECT	41 AS check_id, 
							...
							N'DROP INDEX ['+[schema_name]+'].['+[object_name]+'].['+[index_name]+']',
							index_sanity_id
					FROM	#IndexSanity AS i
			...
			INSERT	#IndexHealthCheckResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
										   secret_columns, index_usage_summary, index_size_summary, create_tsql )
					SELECT	42 AS check_id, 
							...
							i.secret_columns,
							i.index_usage_summary,
							N'DISABLED' AS index_size_summary,
							N'ALTER INDEX ['+[index_name]+'] ON ['+[schema_name]+'].['+[object_name]+'] REBUILD'
					FROM	#IndexSanity AS i
	...
	BEGIN
			...
			INSERT	#IndexHealthCheckResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
					...
					SELECT	60 AS check_id, 
							...
							@WikiBaseAddress + @WikiPage_Abnormal_Psychology AS URL,
			...
			SELECT	71 AS check_id, 
					(SELECT TOP 1 i.index_sanity_id from #IndexSanity i where i.object_id=fk.parent_object_id) as index_sanity_id,
					N'Abnormal Psychology' AS findings_group,
					N'Cascading Updates or Deletes' AS finding, 
					@WikiBaseAddress + @WikiPage_Abnormal_Psychology AS URL,
					QUOTENAME(parent_object_name)+'.'+foreign_key_name
						+ N' has settings:'
	...
	BEGIN
		...
		SELECT TOP 5 
			...
			@WikiBaseAddress + @WikiPage_Workaholics AS URL,
	...
	END
		...
		if @LogToCentral > 0
		BEGIN
			SET @dsql = 'USE dbaperf;DROP TABLE IndexHealth_Results'
			IF OBJECT_ID('dbaperf.dbo.IndexHealth_Results') IS NOT NULL
				EXEC (@dsql);
			...
			SELECT	@rundate	[rundate],
				...
				sn.index_name,
				sn.key_column_names,
				sn.key_column_names_with_sort_order,
				sn.key_column_names_with_sort_order_no_types,
				sn.count_key_columns,
				sn.include_column_names,
				sn.include_column_names_no_types,
				sn.count_included_columns,
				sn.partition_key_column_name,
				sn.filter_definition,
				sn.is_indexed_view,
				sn.is_unique,
				sn.is_primary_key,
				sn.is_XML,
				sn.is_spatial,
				sn.is_NC_columnstore,
				sn.is_CX_columnstore,
				sn.is_disabled,
				sn.is_hypothetical,
				sn.is_padded,
				sn.fill_factor,
				...
				sn.last_user_lookup,
				sn.last_user_update,
				sn.is_referenced_by_foreign_key,
				sn.secret_columns secret_columns_2,
				sn.count_secret_columns,
				sn.create_date,
				sn.modify_date,
				ts.create_tsql AS create_tsql_2,
				dbaadmin.dbo.dbaudf_max_datetime
					(
					dbaadmin.dbo.dbaudf_max_datetime
						(
						dbaadmin.dbo.dbaudf_max_datetime	
							(
							STATS_DATE(sn.[object_id],sn.[index_id])
							,dt.[SQLStartTime]
							)
						,dbaadmin.dbo.dbaudf_max_datetime
							(
			...
			INTO dbaperf.dbo.IndexHealth_Results
			FROM #IndexHealthCheckResults br
			LEFT JOIN #IndexSanity sn ON 
				br.index_sanity_id=sn.index_sanity_id
			LEFT JOIN #IndexCreateTsql ts ON 
				br.index_sanity_id=ts.index_sanity_id
			LEFT JOIN	Dates dt
				ON	sn.[database_name] = dt.DBName

			ORDER BY [check_id] ASC, IHCR_id ASC, findings_group;


			-------------------------------------------------------
			-------------------------------------------------------
			-- EXPORT DATA dbaperf.dbo.IndexHealth_Results
			-------------------------------------------------------
			-------------------------------------------------------

			SET	@Export_Source		= 'dbaperf.dbo.IndexHealth_Results'
			SELECT	@FileName		= REPLACE	(
									[dbaadmin].[dbo].[dbaudf_base64_encode] (
														@@SERVERNAME+'|'+
														REPLACE(@Export_Source,'dbaperf.dbo.','')+'|'+
														COALESCE(@DatabaseName,'NULL')+'|'+
														COALESCE(@SchemaName,'NULL')+'|'+
														COALESCE(@TableName,'NULL')+'|'+
														COALESCE(CONVERT(VarChar(12),@RunDate,101),'NULL'))+
														'.dat'
									,'='
									,'$'
									)
	...
	BEGIN
		...
		UNION ALL
		SELECT 				
			N'dbasp_Diaper_Check(TM) v2.02 - Jan 30, 2014' ,   
			N'Getty Images' ,   
			@WikiBaseAddress + @WikiPage_Index ,
			100000000000,
			N'Thanks from the Operations team.  We hope you found this tool useful, and if you need help relieving your SQL Server pains, contact a DBA.',
...
exec sp_msforEachDB 'IF DB_ID(''?'') > 4 AND ''?'' NOT IN (''dbaadmin'',''dbaperf'',''sqldeploy'') BEGIN PRINT ''PROCESSING ?'';EXEC dbaperf.dbo.dbasp_Diaper_Check @DatabaseName=''?'',@LogToCentral = 2; END'
GO


--SELECT * FROM dbaperf.dbo.IndexHealth_Results
--SELECT * FROM dbaperf_reports.dbo.IndexHealth_Results


-- EXEC dbaperf.dbo.dbasp_Diaper_Check @mode = 0, @DatabaseName = 'msdb'

-- EXEC dbaperf.dbo.dbasp_Diaper_Check @DatabaseName='dbacentral', @SchemaName='dbo', @TableName='DBA_ServerInfo';


--CREATE INDEX [ix_sysjobhistory_job_id_step_id_run_status_includes] ON [msdb].[dbo].[sysjobhistory] ([job_id], [step_id], [run_status])  INCLUDE ([run_date], [run_time], [run_duration]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);

--CREATE INDEX [ix_Filescan_MachineSource_Machine_Domain_includes]				ON [dbacentral].[dbo].[Filescan_MachineSource] ([Machine], [Domain])		INCLUDE ([MachineSourceID]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_Filescan_MachineSource_Domain_includes]					ON [dbacentral].[dbo].[Filescan_MachineSource] ([Domain])			INCLUDE ([MachineSourceID], [Machine]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_Filescan_MachineSource_SourceType_LastReported_includes]			ON [dbacentral].[dbo].[Filescan_MachineSource] ([SourceType], [LastReported])	INCLUDE ([MachineSourceID], [Machine], [Instance]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);


--CREATE INDEX [ix_DBA_DBfileInfo_SQLName_DBName_LogicalName]					ON [dbacentral].[dbo].[DBA_DBfileInfo] ([SQLName], [DBName], [LogicalName])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DBA_JobInfo_SQLName_JobName]							ON [dbacentral].[dbo].[DBA_JobInfo] ([SQLName], [JobName])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_SQLHealth_Central_SQLname_includes]						ON [dbacentral].[dbo].[SQLHealth_Central] ([SQLname])  INCLUDE ([Check_date]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DBA_ClustInfo_SQLName_ResourceType_ResourceName]				ON [dbacentral].[dbo].[DBA_ClustInfo] ([SQLName], [ResourceType], [ResourceName])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_SQLHealth_Central_Check_date_includes]					ON [dbacentral].[dbo].[SQLHealth_Central] ([Check_date])  INCLUDE ([SQLname]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DBA_DBInfo_DBName_BaselineServername_includes]				ON [dbacentral].[dbo].[DBA_DBInfo] ([DBName], [BaselineServername])  INCLUDE ([SQLName], [ENVname], [ENVnum], [BaselineFolder]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DBA_DBInfo_SQLName_status_modDate]						ON [dbacentral].[dbo].[DBA_DBInfo] ([SQLName], [status], [modDate])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DBA_DBInfo_DBName_includes]							ON [dbacentral].[dbo].[DBA_DBInfo] ([DBName])  INCLUDE ([SQLName], [ENVnum]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);


--CREATE INDEX [ix_HealthCheck_current_DBname_Check_type]					ON [DBAadmin].[dbo].[HealthCheck_current] ([DBname], [Check_type])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_IndexMaintenanceProcess_Status]						ON [DBAadmin].[dbo].[IndexMaintenanceProcess] ([Status])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_IndexMaintenanceProcess_Status_includes]					ON [DBAadmin].[dbo].[IndexMaintenanceProcess] ([Status])  INCLUDE ([IMP_ID]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_pong_return_pong_stamp]							ON [DBAadmin].[dbo].[pong_return] ([pong_stamp])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_IndexMaintenanceProcess_Status]						ON [DBAadmin].[dbo].[IndexMaintenanceProcess] ([Status])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_filescan_exclude_useflag]							ON [DBAadmin].[dbo].[filescan_exclude] ([useflag])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_IndexMaintenancePhysicalStats_insert_date_object_id_index_id_page_count]	ON [DBAadmin].[dbo].[IndexMaintenancePhysicalStats] ([insert_date], [object_id], [index_id], [page_count])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);

--CREATE INDEX [ix_unused_objects_work_used_flag_includes] ON [DBAperf].[dbo].[unused_objects_work] ([used_flag])  INCLUDE ([DBname], [Schema_name], [Object_name]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_QueryDaily_current_qs_id] ON [DBAperf].[dbo].[DMV_QueryDaily_current] ([qs_id])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_QueryDaily_current_sql_handle_plan_handle_plan_generation_num_statement_start_offset_dbid_objectid] ON [DBAperf].[dbo].[DMV_QueryDaily_current] ([sql_handle], [plan_handle], [plan_generation_num], [statement_start_offset], [dbid], [objectid])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_DATABASE_FORECAST_DETAIL_RunDate] ON [DBAperf].[dbo].[DMV_DATABASE_FORECAST_DETAIL] ([RunDate])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_DATABASE_FORECAST_DETAIL_RunDate_includes] ON [DBAperf].[dbo].[DMV_DATABASE_FORECAST_DETAIL] ([RunDate])  INCLUDE ([ServerName], [DatabaseName], [DateTimeValue], [Recorded_Smooth], [Forecasted], [Actual]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_unused_objects_DBname_Use_date_includes] ON [DBAperf].[dbo].[unused_objects] ([DBname], [Use_date])  INCLUDE ([Schema_name], [Object_name]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_unused_objects_work_Objectid] ON [DBAperf].[dbo].[unused_objects_work] ([Objectid])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_DRIVE_FORECAST_DETAIL_RunDate] ON [DBAperf].[dbo].[DMV_DRIVE_FORECAST_DETAIL] ([RunDate])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_DRIVE_FORECAST_DETAIL_RunDate_includes] ON [DBAperf].[dbo].[DMV_DRIVE_FORECAST_DETAIL] ([RunDate])  INCLUDE ([ServerName], [DriveLetter], [DateTimeValue], [DriveSize_MB], [CurrentUsed_MB], [ForecastUsed_MB], [Status]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_QueryStats_log_dbid_objectid_rundate] ON [DBAperf].[dbo].[DMV_QueryStats_log] ([dbid], [objectid], [rundate])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_unused_objects_work_used_flag] ON [DBAperf].[dbo].[unused_objects_work] ([used_flag])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_index_stats_log_rundate] ON [DBAperf].[dbo].[index_stats_log] ([rundate])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_dmv_IndexBaseLine_type_desc_row_count_includes] ON [DBAperf].[dbo].[dmv_IndexBaseLine] ([type_desc], [row_count])  INCLUDE ([database_id], [object_id]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_dmv_IndexBaseLine_type_desc_includes] ON [DBAperf].[dbo].[dmv_IndexBaseLine] ([type_desc])  INCLUDE ([row_id], [database_id], [object_id], [indexed_columns]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_QueryStats_current_sql_handle_plan_handle_plan_generation_num_statement_start_offset] ON [DBAperf].[dbo].[DMV_QueryStats_current] ([sql_handle], [plan_handle], [plan_generation_num], [statement_start_offset])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);

--CREATE INDEX [ix_DMV_QueryStats_current_qs_id] ON [DBAperf].[dbo].[DMV_QueryStats_current] ([qs_id])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);

--CREATE INDEX [ix_DMV_DRIVE_FORECAST_DETAIL_RunDate_ServerName] ON [DBAperf_reports].[dbo].[DMV_DRIVE_FORECAST_DETAIL] ([RunDate], [ServerName])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_DMV_DATABASE_FORECAST_SUMMARY_RunDate_ServerName] ON [DBAperf_reports].[dbo].[DMV_DATABASE_FORECAST_SUMMARY] ([RunDate], [ServerName])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_IndexHealth_Results_ServerName_DatabaseName_includes] ON [DBAperf_reports].[dbo].[IndexHealth_Results] ([ServerName], [DatabaseName])  INCLUDE ([rundate]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);

--CREATE INDEX [ix_IndexHealth_Results_ServerName_includes] ON [DBAperf_reports].[dbo].[IndexHealth_Results] ([ServerName])  INCLUDE ([rundate], [DatabaseName], [SchemaName], [TableName]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_IndexHealth_Results_ServerName_includes] ON [DBAperf_reports].[dbo].[IndexHealth_Results] ([ServerName])  INCLUDE ([DatabaseName]) WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);

--CREATE INDEX [ix_IndexHealth_Results_rundate_ServerName_DatabaseName_includes] ON [DBAperf_reports].[dbo].[IndexHealth_Results] ([rundate], [ServerName], [DatabaseName])  INCLUDE ([SchemaName], [TableName], [blitz_result_id], [check_id], [findings_group], [finding], [URL], [details], [index_definition], [secret_columns], [index_usage_summary], [index_size_summary], [create_tsql], [more_info], [database_id], [object_id], [index_id], [index_type], [database_name], [schema_name], [object_name], [index_name], [key_column_names], [key_column_names_with_sort_order], [key_column_names_with_sort_order_no_types], [count_key_columns], [include_column_names], [include_column_names_no_types], [count_included_columns], [partition_key_column_name], [filter_definition], [is_indexed_view], [is_unique], [is_primary_key], [is_XML], [is_spatial], [is_NC_columnstore], [is_CX_columnstore], [is_disabled], [is_hypothetical], [is_padded], [fill_factor], [user_seeks], [user_scans], [user_lookups], [user_updates], [last_user_seek], [last_user_scan], [last_user_lookup], [last_user_update], [is_referenced_by_foreign_key], [secret_columns_2], [count_secret_columns], [create_date], [modify_date], [create_tsql_2]) WITH (FILLFACTOR=100, ONLINE=OFF, SORT_IN_TEMPDB=ON);
--CREATE INDEX [ix_IndexHealth_Results_object_name] ON [DBAperf_reports].[dbo].[IndexHealth_Results] ([object_name])  WITH (FILLFACTOR=100, ONLINE=ON, SORT_IN_TEMPDB=ON);
