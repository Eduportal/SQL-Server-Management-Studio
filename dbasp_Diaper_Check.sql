USE [DBAperf]
GO
/****** Object:  StoredProcedure [dbo].[dbasp_Diaper_Check]    Script Date: 6/9/2015 11:34:55 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[dbasp_Diaper_Check]
...
============================================================================================================================
06/09/2015	Steve Ledridge		Modified Table output to not use "MAX" blob columns to improve performance
	...
	BEGIN
		...
		CREATE TABLE #IndexHealthCheckResults
			(
			  ...
			  index_definition NVARCHAR(4000) NOT NULL,
			  secret_columns NVARCHAR(1000) NULL,
			  index_usage_summary NVARCHAR(1000) NULL,
			  index_size_summary NVARCHAR(1000) NULL,
			  create_tsql NVARCHAR(4000) NULL,
			  more_info NVARCHAR(1000)NULL
			);
		...
		CREATE TABLE #IndexSanity
			(
			  ...
			  key_column_names NVARCHAR(1000) NULL ,
			  key_column_names_with_sort_order NVARCHAR(1000) NULL ,
			  key_column_names_with_sort_order_no_types NVARCHAR(1000) NULL ,
			  count_key_columns INT NULL ,
			  include_column_names NVARCHAR(4000) NULL ,
			  include_column_names_no_types NVARCHAR(4000) NULL ,
			  count_included_columns INT NULL ,
			  partition_key_column_name NVARCHAR(1000) NULL,
			  filter_definition NVARCHAR(1000) NOT NULL ,
...
BEGIN;
	...
	END
		...
		BEGIN
			...
			-- SETUP NULLABILITY AND PK
			exec('ALTER TABLE  [DBAperf].[dbo].[IndexHealth_Results] ALTER COLUMN [rundate] DateTime Not Null')
			exec('ALTER TABLE  [DBAperf].[dbo].[IndexHealth_Results] ALTER COLUMN [ServerName] nVarChar(128) Not Null')
			exec('ALTER TABLE  [DBAperf].[dbo].[IndexHealth_Results] ALTER COLUMN [DatabaseName] nVarChar(128) Not Null')
			exec('ALTER TABLE  [DBAperf].[dbo].[IndexHealth_Results] ALTER COLUMN [IHCR_id] Int Not Null')

			exec('ALTER TABLE [DBAperf].[dbo].[IndexHealth_Results] ADD  CONSTRAINT [PK_IndexHealth_Results] PRIMARY KEY CLUSTERED 
				(
				[ServerName] ASC,
				[DatabaseName] ASC,
				[rundate] ASC,
...
-- RUN ON 1 DATABASE AND LOG TO CENTRAL SERVER
-- EXEC dbaperf.dbo.dbasp_Diaper_Check @DatabaseName='DBName',@LogToCentral = 2;

-- RUN ON ALL DATABASES  AND LOG TO CENTRAL SERVER (THIS IS USED IN AGENT JOB)
