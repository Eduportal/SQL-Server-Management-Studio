...
FROM		(
		...
		FROM		(
				SELECT		'CREATE NONCLUSTERED INDEX AMIX_' + object_name(c.object_id) +'_'+ left(cast(newid() as varchar(500)),5) 
...
FROM		(
		SELECT		'DROP INDEX ' + QUOTENAME(i.name) + ' ON ' + QUOTENAME(s.name) + '.' + QUOTENAME(OBJECT_NAME(dm_ius.OBJECT_ID)) AS [drop_statement]
		...
		WHERE		OBJECTPROPERTY(dm_ius.OBJECT_ID,'IsUserTable') = 1
			...
			AND	coalesce(last_user_seek,last_user_scan,last_user_lookup,last_system_scan,last_system_seek,last_system_lookup,'01-01-1980') < GetDate()-30
