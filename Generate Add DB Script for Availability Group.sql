...
DECLARE		@DBName			SYSNAME
		,@SourceServer		SYSNAME		= 'ASHPSQLVCTR01'
		,@DestServer		SYSNAME		= 'ASHPSQLVCTR02'
		,@AGroup		SYSNAME		= NULL
		,@ExcludeDBs		VarChar(max)	= '' -- Pipe Delimited Database List
		,@InAG			CHAR(1)
		,@AllDBsJoined		BIT
...
RAISERROR (''Generating Full Backup Command on Database '+CHAR(36)+'(DBName).'',-1,-1) WITH NOWAIT
...
RAISERROR (''Backing Up Database '+CHAR(36)+'(DBName).'',-1,-1) WITH NOWAIT
EXEC (@Backup_cmd)
GO
WAITFOR DELAY ''00:01:00''
GO
...
DECLARE @Backup_cmd nvarchar(max)
RAISERROR (''Generating Diff Backup Command on Database '+CHAR(36)+'(DBName).'',-1,-1) WITH NOWAIT
EXEC [dbaadmin].[dbo].[dbasp_format_BackupRestore] 
		@DBName			= '''+CHAR(36)+'(DBName)''
		,@Mode			= ''BD'' 
		,@Verbose		= 0
