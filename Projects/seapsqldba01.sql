...
SELECT	@CheckDate					= GetDate()
		...
		,@save_Redgate_flag			= 'n'
		,@isNMinstance				= 'n'
		,@EnableCodeComments		= CASE @Verbose WHEN -1 THEN 1 ELSE 0 END
...
-- SET JOB STATUS CALCULATION VALUES
SET			@FilterOutlierPercent	= 10
SET			@FloatRange				= 100
SET			@Today					= CAST(CONVERT(VarChar(12),@CheckDate,101)AS DATETIME)
...
SELECT		[subject01]
			,[value01]
			,CASE WHEN [notes01] IS NULL THEN 'Pass' ELSE 'Fail' END [grade01]
			,COALESCE([notes01],'') [notes01]
FROM		(			
...
FROM		(
			...
			JOIN		msdb..sysjobs T2
					ON	T1.job_id = T2.job_id
			WHERE		Enabled = 1 AND output_file_name NOT LIKE @LogPath +'%'	
