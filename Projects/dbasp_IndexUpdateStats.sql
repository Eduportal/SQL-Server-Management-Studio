...
GO
ALTER PROCEDURE [dbo].[dbasp_IndexUpdateStats]
	(
...
-------------------------------------------------------
-- Updates statistics against a database.
-------------------------------------------------------
...
BEGIN
	...
	FETCH NEXT FROM CreateAllDBViews INTO @S,@T
	WHILE (@@fetch_status <> -1)
	BEGIN
		IF (@@fetch_status <> -2)
		BEGIN
			SET	@TSQL	= 'IF OBJECT_ID(''[dbo].[vw_AllDB_'+@T+']'') IS NOT NULL' +CHAR(13)+CHAR(10)
			+ 'DROP VIEW [dbo].[vw_AllDB_'+@T+']' +CHAR(13)+CHAR(10)
			SET	@TSQL	= 'USE [dbaperf];'+CHAR(13)+CHAR(10)+'EXEC (''' + REPLACE(@TSQL,'''','''''') + ''')'
			EXEC	(@TSQL)

			SET	@TSQL	= 'CREATE VIEW [dbo].[vw_AllDB_'+@T+'] AS' +CHAR(13)+CHAR(10)+'SELECT	''master'' AS database_name, DB_ID(''master'') AS database_id, * From [master].['+@S+'].['+@T+']'+CHAR(13)+CHAR(10)
			SELECT	@TSQL = @TSQL +
			'UNION ALL'+CHAR(13)+CHAR(10)+'SELECT	'''+name+''', DB_ID('''+name+'''), * From ['+name+'].['+@S+'].['+@T+']'+CHAR(13)+CHAR(10)
...
BEGIN
	...
	SET @lMsg = CAST(@@ServerName + N':' AS NCHAR(25)) + N'Starting statistics maintenance' 
	...
	SET @lMsg = CAST(N'@ProcessGUID:' AS NCHAR(25)) + N'Using Generated Proccess GUID.'
	EXEC [dbo].[dbasp_LogMsg]
		@ModuleName=@cEModule
		,@MessageKeyword=@cEMessage
...
BEGIN
	...
	SET @lMsg = CAST(N'@ProcessGUID:' AS NCHAR(25)) + N'Using Proccess GUID Passed to Procedure.'
	EXEC [dbo].[dbasp_LogMsg]
		@ModuleName=@cEModule
		,@MessageKeyword=@cEMessage
		,@TypeKeyword='EVT_INFO'
...
BEGIN TRY -- Outer Try block
		...
		SET	@lMsg	= CASE
				WHEN	@databaseName IS NOT NULL 
				 AND	@databaseName NOT IN (SELECT [name] FROM sys.databases WITH(nolock))
					THEN CAST(N'@databaseName:' AS NCHAR(25)) + N'DB NAME WAS SPECIFIED BUT NOT ACCURATE'
				WHEN	coalesce(@ScriptMode,-1) not in (0,1)	
					THEN CAST(N'@ScriptMode:' AS NCHAR(25)) + N'Parameter is Invalid'
				WHEN	coalesce(@noRecompute,-1) not in (0,1)	
					THEN CAST(N'@noRecompute:' AS NCHAR(25)) + N'Parameter is Invalid'	
				WHEN	coalesce(@rowsUpdatedThreshold,-2) < -1	
					THEN CAST(N'@rowsUpdatedThreshold:' AS NCHAR(25)) + N'Parameter is Invalid'	
				WHEN	@sampling not between 0 and 100	
					THEN CAST(N'@sampling:' AS NCHAR(25)) + N'Parameter is Invalid'	
				ELSE	NULL END	
	...
	IF Exists (select * From dbaperf.dbo.vw_AllDB_stats where no_recompute = 1) -- DEFAULT IS 0
	BEGIN
		SET @lMsg = CAST(N'ALERT:' AS NCHAR(25)) + N'One or More Statistics are set to NoRecompute.'
		exec [dbo].[dbasp_LogMsg]
			@ModuleName=@cEModule
			,@MessageKeyword=@cEMessage
			,@TypeKeyword='EVT_FAIL'
	...
	BEGIN
		SET @lMsg = CAST(N'ALERT:' AS NCHAR(25)) + N'Auto_Create_Stats is turned off on one or more databases.'
		exec [dbo].[dbasp_LogMsg]
			@ModuleName=@cEModule
			,@MessageKeyword=@cEMessage
			,@TypeKeyword='EVT_FAIL'
	...
	BEGIN
		SET @lMsg = CAST(N'ALERT:' AS NCHAR(25)) + N'Auto_update_stats is turned off on one or more databases.'
		exec [dbo].[dbasp_LogMsg]
			@ModuleName=@cEModule
			,@MessageKeyword=@cEMessage
			,@TypeKeyword='EVT_FAIL'
	...
	BEGIN
		SET @lMsg = CAST(N'ALERT:' AS NCHAR(25)) + N'Auto_update_stats_async is turned on on one or more databases.'
		exec [dbo].[dbasp_LogMsg]
			@ModuleName=@cEModule
			,@MessageKeyword=@cEMessage
			,@TypeKeyword='EVT_FAIL'
	...
	SET @lMsg = CAST(N'Cursor:' AS NCHAR(25)) + N'Getting tables with qualifying statistics.'
	EXEC [dbo].[dbasp_LogMsg]
		@ModuleName=@cEModule
		,@MessageKeyword=@cEMessage
		,@TypeKeyword='EVT_INFO'
	...
	FROM		(
			...
			FROM		dbaperf.dbo.vw_AllDB_objects ob
			JOIN		dbaperf.dbo.vw_AllDB_schemas sc
			...
			LEFT join	dbaperf.dbo.vw_AllDB_sysindexes si		-- get rowmodctr from this table
			...
			LEFT JOIN		dbaperf.dbo.vw_AllDB_indexes i 
	...
	BEGIN --block for WHILE @@FETCH_STATUS = 0 and @lError=0
		BEGIN TRY -- try block within cursor
			...
			IF @LastDB IS NULL
			BEGIN
			set @lMsg= CAST(@DatabaseName + N':' AS NCHAR(25)) + N'Starting Database.';
			exec [dbo].[dbasp_LogMsg]
				@ModuleName=@cEModule
				,@MessageKeyword=@cEMessage
				,@TypeKeyword='EVT_START'
			...
			BEGIN
				...
				BEGIN
					set @lMsg= CAST(@LastDB + N':' AS NCHAR(25)) + N'Finishing Database.';
					exec [dbo].[dbasp_LogMsg]
						@ModuleName=@cEModule
						,@MessageKeyword=@cEMessage
						,@TypeKeyword='EVT_END'
			...
			BEGIN
				set @lMsg=CAST(N'Output:' AS NCHAR(25)) +N'SETTING USE ' + QUOTENAME(COALESCE(@databaseName,'??????????')) + N' IN OUTPUT.';
				exec [dbo].[dbasp_LogMsg]
					@ModuleName=@cEModule
					,@MessageKeyword=@cEMessage
					,@TypeKeyword='EVT_INFO'
			...
			set @lMsg= CAST(N'Evaluating:' AS NCHAR(25)) + @qualifiedTableName + N'.';
			exec [dbo].[dbasp_LogMsg]
				@ModuleName=@cEModule
				,@MessageKeyword=@cEMessage
				,@TypeKeyword='EVT_INFO'
			...
			IF @daysSinceStatsLastUpdated  is null
				BEGIN --If statistics have not been created anywhere on the table, do nothing but log it.
					SET @lMsg=CAST('' AS NCHAR(25)) + @qualifiedTableName + N' has not had statistics created.';
					exec [dbo].[dbasp_LogMsg]
						@ModuleName=@cEModule
						,@MessageKeyword=@cEMessage
						,@TypeKeyword='EVT_SKIP'
						,@ProcessGUID=@ProcessGUID
						,@AdHocMsg = @lMsg 
						,@Diagnose=@Diagnose
			...
			ELSE IF @daysSinceStatsLastUpdated  <= @updateStatsSinceDays 
				BEGIN --If statistics have been updated within the amount of time specified, do nothing but log it.
					SET @lMsg=CAST('' AS NCHAR(25)) + @qualifiedTableName + N' was last updated ' + cast(@daysSinceStatsLastUpdated as nvarchar) 
			...
			ELSE if @rowModCtr < @rowsUpdatedThreshold 
				begin --If the rowModCtr is less than our threshold, do nothing but log it.
					set @lMsg=CAST('' AS NCHAR(25)) + @qualifiedTableName + N' has only had ' 
			...
			-------------------------------------------------------
				begin -- Begin block to update statistics
					...
					-- ADD COMMENTS
					SET @updateStatsCommand = CAST(COALESCE(@updateStatsCommand,'') AS CHAR(70)) + '-- '
						+ 'Last updated ' + cast(@daysSinceStatsLastUpdated as nvarchar) + N' days ago, '
						+ 'had ' + cast(@rowModCtr as nvarchar) + N' row(s) changed.'

					-- Log the command 
					set @lMsg = case @ScriptMode when 1 then CAST('Output:' AS NCHAR(25)) else CAST('' AS NCHAR(25))  end + @updateStatsCommand
		...
		BEGIN CATCH -- Catch for try block within cursor
			SELECT @lMsg = CAST('try/catch:' AS NCHAR(25)) + N'dbasp_IndexUpdateStats failed againt ' 
