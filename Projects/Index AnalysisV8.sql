...
SELECT	@@Servername
	...
	+ CASE
	  WHEN equality_columns IS NOT NULL AND inequality_columns IS NULL THEN ''
	  ELSE ', '
	  END
	+ COALESCE(inequality_columns,'') as indexed_columns
...
SELECT		T1.server_name
		...
		,T1.index_action
		,T1.index_name
		,T2.IndexName AS new_index_name
		,T1.indexed_columns
		,T1.included_columns
		,T1.is_unique
		,T1.type_desc
...
FROM		#IndexBaseLine T1
LEFT JOIN	@CahedPlanMissingIndexes T2
	ON	T1.indexed_columns = T2.[IndexColumns]
	AND	T1.included_columns = T2.[IncludeColumns]
	AND	T1.index_name = '--MISSING--'
	
ORDER BY	index_action
		,type_desc
		,impact_estimate_curent desc	
	
--ORDER BY T1.table_buffer_mb DESC,  T1.user_total_read DESC
