...
IF OBJECT_ID ('[dbo].[dbasp_SQL_Server_System_Report]') IS NOT NULL
BEGIN

	DROP PROCEDURE [dbo].[dbasp_SQL_Server_System_Report]
...
CREATE PROCEDURE [dbo].[dbasp_SQL_Server_System_Report]
...
SET @Path = '\\' + REPLACE(@@SERVERNAME,@@SERVICENAME,'')+'\'+REPLACE(@@SERVERNAME,'\','$')+'_dbasql\dba_reports\'
...
SET @FileName = 'SSSR_'+REPLACE(@@SERVERNAME,'\','$')+'.html'
SET @vXML_String = ''
SET @vBody = ''
...
IF OBJECT_ID ('tempdb.dbo.#instance_property_temp') IS NOT NULL
BEGIN

	DROP TABLE dbo.#instance_property_temp

END


IF OBJECT_ID ('tempdb.dbo.#database_size_distribution_stats_temp') IS NOT NULL
BEGIN

	DROP TABLE dbo.#database_size_distribution_stats_temp

END


IF OBJECT_ID ('tempdb.dbo.#model_compatibility_size_growth_temp') IS NOT NULL
BEGIN

	DROP TABLE dbo.#model_compatibility_size_growth_temp

END


IF OBJECT_ID ('tempdb.dbo.#last_backup_set_temp') IS NOT NULL
BEGIN

	DROP TABLE dbo.#last_backup_set_temp

END


IF OBJECT_ID ('tempdb.dbo.#agent_jobs_temp') IS NOT NULL
BEGIN

	DROP TABLE dbo.#agent_jobs_temp

END


IF OBJECT_ID ('tempdb.dbo.#unused_indexes_temp') IS NOT NULL
BEGIN

	DROP TABLE dbo.#unused_indexes_temp
...
CREATE TABLE dbo.#database_size_distribution_stats_temp
...
BEGIN
	...
	SET @vSQL_String =
		...
		'
			...
			INSERT INTO dbo.#database_size_distribution_stats_temp
...
IF (SELECT COUNT (*) FROM dbo.#database_size_distribution_stats_temp) = 0
BEGIN
...
SET @vXML_String =
	...
	CONVERT (NVARCHAR (MAX),
		(
			...
			FROM
				dbo.#database_size_distribution_stats_temp X
			ORDER BY
				X.database_name
			FOR
				XML PATH ('tr')
...
INTO
	dbo.#model_compatibility_size_growth_temp
FROM
...
SET @vXML_String =
	...
	CONVERT (NVARCHAR (MAX),
		(
			...
			FROM
				dbo.#model_compatibility_size_growth_temp X
			ORDER BY
				 X.database_name
...
BEGIN
	...
	SET @vSQL_String =
		...
		'
			...
			INSERT INTO dbo.#unused_indexes_temp
...
IF (SELECT COUNT (*) FROM dbo.#unused_indexes_temp) = 0
BEGIN
