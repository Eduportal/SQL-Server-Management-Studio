...
-- EXEC [dbo].[dbasp_SQL_Server_System_Report] 'E'
...
IF OBJECT_ID (N'[dbo].[dbasp_SQL_Server_System_Report]', N'P') IS NOT NULL
BEGIN
...
SET @vRecipients = 'steve.ledridge@gettyimages.com'
...
BEGIN
	...
	EXEC dbaadmin.dbo.dbasp_FileAccess_Write @vBody,'c:\ServerReport.html',0,1


	EXECUTE [dbaadmin].[dbo].[dbasp_sendmail]

		 @recipients = @vRecipients
		,@copy_recipients = @vCopy_Recipients
		,@subject = @vSubject
		,@message = 'Server Report Attached'
		,@attachments = 'c:\ServerReport.html'
END
GO
