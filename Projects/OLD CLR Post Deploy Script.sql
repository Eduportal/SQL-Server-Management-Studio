...
BEGIN	------	GENERATE COMMENT BLOCK EXTENDED PROPERTIES		
	...
	SELECT		@Script = 'PRINT N''Creating [GettyImages.Operations.CLRTools]...'';'+@CRLF+'GO'+@CRLF+@CRLF+$(DatabaseName).dbo.dbaudf_ExtractClrDdl (@DOC_SQL,'ASSEMBLY',NULL,'GettyImages.Operations.CLRTools')+@CRLF
	...
	WITH	------	GENERATE CURSOR WITH ALL DATA FOR SCRIPTING			
			...
			(
			SELECT		MemberName
					,[Script]	= $(DatabaseName).dbo.dbaudf_ExtractClrDdl (@DOC_SQL,[Object_Type],[Object_Schema],[Object_Name])
			FROM		ObjectInfo
			)
			,CommentBlock
			AS
	...
	BEGIN
		...
		BEGIN
			SELECT		@Script = 'PRINT N''Creating ['+@Object_Schema+'].['+@Object_Name+']...'';'+@CRLF
					...
					+ 'SET	@HeaderXML = ''' + $(DatabaseName).dbo.dbaudf_FormatXML2String(@CommentHeader) + ''''+@CRLF+@CRLF


			-- WRITING TO OUTPUT FILE APPENDING TO WHAT ALREADY EXISTED THERE
			-- WRITING CREATE OBJECT SECTION 1
			EXEC $(DatabaseName).dbo.dbasp_FileAccess_Write @Script,@FileName_OUT,1,1
