   ...
   BEGIN						-- MANUALY RUN
	...
	IF @error_count > 0								-- CHECK: ERROR COUNT SO FAR
	   BEGIN
		...
		-- RAISERROR AND EXIT --
		SELECT	@miscprint			= 'Error: Manual Execution Failed Because of Invalid Parameters'
...
If DB_ID(@CL_DBName) IS NULL	-- CHECK: DBNAME
   BEGIN
	SELECT	@miscprint	= 'ERROR: Control row for this Restore job step could not be found (Server ' + @@SERVERNAME + ').'
			,@error_count = @error_count + 1										-- SET IT
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT IT

	-- RAISERROR AND EXIT --
	SELECT	@miscprint			= 'Error: Automated Execution Failed Because of Invalid or Missing Control_Local Record'
			,@NestLevel			= @NestLevel + 1
			,@TerminatedEarly	= 1													-- SET IT
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT IT
	EXEC	[dbo].[dbasp_print] '-------------------------------------FORCED ERROR----------------------------------------',0,0,1
...
BEGIN							-- CREATE LOG FILES													
	...
	CreateLog:
		...
		IF [dbo].[dbaudf_GetFileProperty] (@Reusable_String_2,'FIle','Path') IS NOT NULL
		   BEGIN
			SELECT	@miscprint = 'CREATED:  '+@Reusable_String_1+' File: ' + @Reusable_String_2 + @CRLF
			EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1
...
IF								-- CHECK: BUILD CODE DIRECTORY										
   ...
   BEGIN
	SELECT	@miscprint = 'Path to Build Folder [' + @CT_DB_Copy_BuildCode + '] is NOT Valid'
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1
...
Else
   BEGIN
	SELECT	@miscprint = 'Build Folder [' + @currpath + '] WILL Be Used For This SQL Deployment.'
...
IF @ReplEnabled = 1				-- CHECK: REPLICATION												
   BEGIN
	-- RAISERROR AND EXIT --
	SELECT	@miscprint			= 'Error:  Replication (table) Detected in database ' + @DBname + '.'
			,@error_count		= @error_count + 1
			,@TerminatedEarly	= 1
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1													-- PRINT
	EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1								-- APPEND LOG
...
BEGIN							-- CHECK: ReleaseNotes.doc											
	...
	IF [dbo].[dbaudf_GetFileProperty] (@Reusable_String_1,'File','Path') IS NULL
	   BEGIN						-- FAIL
		SELECT	@miscprint		= 'Note: ReleaseNotes.doc WAS NOT Found in ' + @currpath
		EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1										-- PRINT
		EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1					-- APPEND LOG
	   END
	Else
	   BEGIN						-- PASS
		...
		SELECT	@miscprint		= 'Note: ReleaseNotes.doc Located.'
		EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1										-- PRINT IT
		EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1					-- LOG IT
	   END
	SELECT	@Reusable_String_1	= NULL
...
BEGIN							-- CHECK: DBA_CHANGELIST.TXT										
	...
	ELSE
	BEGIN
		SELECT	@miscprint			= 'Note: DBA_ChangeList.txt Located.'
		EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1										-- PRINT
		EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1					-- APPEND LOG
	END
	SELECT	@Reusable_String_1	= NULL		
...
BEGIN							-- START: WRITING LOG HEADER										
	...
	SELECT	@miscprint = '* Logging to ' + @log_file_name + '.log'
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1											-- PRINT
	EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1						-- APPEND LOG
	...
	EXEC	[dbo].[dbasp_RunTSQL]
			...
			,@DBName			= @DBname
			,@Server			= @@Servername
			,@OutputText		= @miscprint
			,@StartNestLevel	= @NestLevel
			,@SQLcmdOptions		= ' -I -b'

	SELECT	@miscprint = STUFF(@miscprint,1,CHARINDEX('Running',@miscprint)-1,'')				-- SET IT
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1										-- PRINT IT
	EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1					-- LOG IT
...
BEGIN							-- LOAD:  UPDATEBUILD.TXT											
	...
	SELECT	@BuildLabel			= REPLACE(COALESCE(COALESCE(LTRIM(RTRIM(@version)) + '_','') 
								+ CASE COALESCE(LTRIM(RTRIM(@rev)),'') WHEN '' THEN LTRIM(RTRIM(@label)) ELSE LTRIM(RTRIM(@label))
								+ '_' + LTRIM(RTRIM(@rev)) END,'unknown'),' ','')
			,@start_datetime	= getdate()
			,@miscprint			= 'Building Build Label. ('+ LTRIM(RTRIM(@BuildLabel)) +')'
			,@NestLevel			= @NestLevel + 1												-- SET IT
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1										-- PRINT IT
	EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1					-- LOG IT
...
BEGIN							-- Headers for Start of the Build Database Section					
	...
	SELECT	@miscprint = 'START BUILDING DATABASE OBJECTS ON ' + upper(@@servername) + ' DB=' + @dbname 
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1
	EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1 -- APPEND

	SELECT	@miscprint = 'IN THE ' + upper(@EI_ENVName) + ' ENVIRONMENT'
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1
	EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1 -- APPEND
...
If @CL_BuildType = 1			-- Insert sprocs-only record into build_files						
   begin
	...
	SELECT	@miscprint = 'A sprocs-only build will be done.'
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1
	EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1 -- APPEND
   end
Else
...
BEGIN							--													
	...
	BEGIN						--
		BEGIN						-- PRINT AND LOG									
			SELECT	@miscprint = CONVERT(VarChar(20),GetDate(),114)+ ': Starting  ' + @DeplStage + ' Stage...'
					,@NestLevel = 0
			EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1										-- PRINT
			EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1					-- APPEND LOG
			SELECT	@NestLevel = @NestLevel + 1
		...
		BEGIN					-- DEPL Stage SKIPS
			...
			BEGIN							-- PRINT/LOG/SKIP							
				SELECT	@miscprint	= CONVERT(VarChar(20),GetDate(),114) + ': ' + @DeplStage + ' Folder was not Expected, Stage Was Skiped...'
				EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1								-- PRINT
				EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1			-- APPEND LOG	
			...
			BEGIN							-- PRINT/LOG/SKIP							
				SELECT	@miscprint	= CONVERT(VarChar(20),GetDate(),114) + ': ' + @DeplStage + ' Folder was Expected, But NOT Found...'
				EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1								-- PRINT
				EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1			-- APPEND LOG	
			...
			BEGIN
				SELECT	@miscprint = CONVERT(VarChar(20),GetDate(),114)+ ': ' + @EI_ENVName + ' Environment, ' + @DeplStage + ' Stage Was Skiped...'
				EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1								-- PRINT
				EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1			-- APPEND LOG
				GOTO DeplStageComplete
			END
		...
		BEGIN					-- STAGE HEADER										
			...
			SELECT	@miscprint = CONVERT(VarChar(20),GetDate(),114)+ ': RUNNING ' + @DeplStage
			EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1										-- PRINT
			EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1					-- APPEND LOG
			...
			BEGIN							--
				...
				BEGIN						--
					BEGIN					-- START:		FILE HEADER				
						...
						END
						BEGIN		-- PRINT AND LOG									
							SELECT	@miscprint = 'SQL FILE: ' + REPLACE(@File_Path,@currfolder,'') 
							EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT
							EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG
						END
					END
						...
						IF		-- CHECK:	StageType								
							...
							BEGIN	-- BCP IN DATA								
								...
								BEGIN		-- PRINT:	AND LOG IT										
									EXEC	[dbo].[dbasp_print] @bcpcmd,@NestLevel,0,1								-- PRINT
									EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG
								END
								EXEC	-- BCP:		DO IT											
							...
							BEGIN	-- COPY FILES								
								...
								IF		-- CHECK:	CREATE SUBFOLDER						
									...
									BEGIN
										...
										EXEC	[dbo].[dbasp_print] @cmd,@NestLevel,0,1										-- PRINT
										EXEC @Result = master.sys.xp_cmdshell @cmd, no_output										
										IF	-- FAIL:	FOLDER DOES NOT EXIST AND CANT BE CREATED	
											@Result != 0	
											BEGIN	
												SELECT	@miscprint = 'ERROR: FOLDER ['+@Reusable_String_1+'], DOES NOT EXIST BUT COULD NOT BE CREATED.' 
														,@error_count = @error_count + 1
												EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT
												EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG												
												EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name_E, NULL, 1	-- APPEND ERROR												
										...
										ELSE
											BEGIN	
												SELECT	@miscprint = 'Created Folder ['+@Reusable_String_1+'], WAS CREATED.' 
												EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1									-- PRINT
												EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1				-- APPEND LOG																		
											END
									END
								...
								IF		-- FAIL:	XCOPY RETURNED AN ERROR					
									@Result != 0	
									BEGIN	
										SELECT	@miscprint = 'ERROR: XCOPY Failed to copy ['+@File_Path+'] to ['+@Reusable_String_1+']' 
												,@error_count = @error_count + 1
										EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT
										EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG												
										EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name_E, NULL, 1	-- APPEND ERROR												
							...
							BEGIN	-- EXECUTE SQL									
								...
								BEGIN	-- LOG: TO OUTPUT FILES								
									...
									BEGIN
										...
										EXEC	[dbo].[dbasp_FileAccess_Write] @OutputText, @log_file_name_E, NULL, 1	-- APPEND ERROR
									END
								END
							END   						
					...
					IF						-- CHECK:		RESULT STATUS			
					   ...
					   BEGIN		-- FAILURE										
						...
						BEGIN		-- START:		FAILURE HEADER					
							SELECT	@miscprint	= 'ERROR: ' + convert(varchar(50), @start_datetime, 120) +CHAR(9)+CHAR(9)+ convert(varchar(50), @end_datetime, 120) +CHAR(9)+CHAR(9)+ 'Duration: ' + CAST(@run_duration_ss AS VarChar(50)) +' Seconds.'
							EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT
							EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG
							EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name_E, NULL, 1	-- APPEND ERROR
						END
					...
					ELSE BEGIN		-- SUCCESS										
							...
							IF						-- CHECK:		LONG RUNNING SCRIPT		
							   ...
							   BEGIN		-- WARNING										
								SELECT	@miscprint = '    -- WARNING: [' +@Reusable_String_2+ '] Long Running Script ran for ' + CAST((@run_duration_ss / 60.0) AS VarChar(10)) + ' minutes.'
										,@warn_count = @warn_count + 1
								EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT
								EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG
								EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name_W, NULL, 1	-- APPEND WARN
								SELECT	@miscprint	= 'WARN:  ' + convert(varchar(50), @start_datetime, 120) +CHAR(9)+CHAR(9)+ convert(varchar(50), @end_datetime, 120) +CHAR(9)+CHAR(9)+ 'Duration: ' + CAST(@run_duration_ss AS VarChar(50)) +' Seconds.'
								EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT
								EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG
								EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name_W, NULL, 1	-- APPEND LOG
							   END
							ELSE
							   BEGIN
								SELECT	@miscprint	= 'OK:    ' + convert(varchar(50), @start_datetime, 120) +CHAR(9)+CHAR(9)+ convert(varchar(50), @end_datetime, 120) +CHAR(9)+CHAR(9)+ 'Duration: ' + CAST(@run_duration_ss AS VarChar(50)) +' Seconds.'
								EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1							-- PRINT
								EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG
							   END
					END
					...
					--	--EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1		-- APPEND LOG
					--	--IF @Result != 0
					--	--	EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name_E, NULL, 1	-- APPEND LOG
					--END
...
IF @Reusable_BIT_1 = 1			-- RUN FINALIZATION PROCESS											
	BEGIN		
	  BEGIN						-- PRINT & LOG FINALIZATION PROCESS HEADER				
		SELECT	@NestLevel	= @NestLevel + 1
				,@miscprint = REPLICATE('=',75)
											+ 'RUNNING:		' + @Reusable_String_1 + '....'
											+ REPLICATE('=',75) 
		...
		SELECT	@miscprint = REPLICATE('=',75)
											+ 'SQL SPROC:	DEPLinfo.dbo.' + @Reusable_String_1
											+ REPLICATE('=',75) 
											+ convert(varchar(50), getdate(), 120)
		EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1											-- PRINT
		EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1						-- APPEND LOG
		...
		BEGIN				-- EXECUTE SCRIPT										
			...
			BEGIN			-- LOG: TO OUTPUT FILES					
				...
				BEGIN			-- PRINT AND LOG ERROR
					...
					SELECT	@miscprint = REPLICATE('=',75)
														+ 'ERROR:	DEPLinfo.dbo.'+ @Reusable_String_1 +' returned an error.'
														+ REPLICATE('=',75)
														+ convert(varchar(50), getdate(), 120)
					EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1											-- PRINT
					EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1						-- APPEND LOG
				...
				BEGIN			-- PRINT AND LOG SUCCESS									
					SELECT	@miscprint	= REPLICATE('=',75)
										+ 'SUCCESS:	DEPLinfo.dbo.'+ @Reusable_String_1 +' Completed.'
										+ REPLICATE('=',75)
										+ convert(varchar(50), getdate(), 120)
					EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1											-- PRINT
					EXEC	[dbo].[dbasp_FileAccess_Write] @miscprint, @log_file_name, NULL, 1						-- APPEND LOG
...
BEGIN
	...
	SELECT	@miscprint	= REPLICATE('=',75)
						+ 'FAILURE:		Deployment Terminated Early and Not All Code Was Deployed.'
						+ REPLICATE('=',75)
						+ convert(varchar(50), getdate(), 120)
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1											-- PRINT
...
BEGIN
	...
	SELECT	@miscprint	= REPLICATE('=',75)
						+ 'ERRORS:		'+CAST(@error_count as varchar)+' Errors were identified durring this deployment.'
						+ REPLICATE('=',75)
						+ convert(varchar(50), getdate(), 120)
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1											-- PRINT
	IF @log_file_name_E IS NOT NULL
...
BEGIN
	...
	SELECT	@miscprint	= REPLICATE('=',75)
						+ 'WARNINGS:	'+CAST(@warn_count as varchar)+' Warnings were identified durring this deployment'
						+ REPLICATE('=',75)
						+ convert(varchar(50), getdate(), 120)
	EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1											-- PRINT
...
Select	@miscprint	= 'CONTEXT ID: CLEARING '												-- SET IT
					+ CAST(CAST(@ContextInfo AS UniqueIdentifier) AS VarChar(1024))
EXEC	[dbo].[dbasp_print] @miscprint,@NestLevel,0,1										-- PRINT IT
