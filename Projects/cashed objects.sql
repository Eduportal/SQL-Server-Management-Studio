--sp_msforeachdb 'select ''?'',object_name(id),* From ?..syscomments where text like ''%VitriaEventLog_sdw_view%'' order by 1'
...
select
       ...
       -- ,sum(t_avg_worker_time) as [avg cpu time]
        ,SUM(t_execution_count) as [execution_count]
from sql_handle_convert_table
where t_obj_name in (select object_name(id) From syscomments where text like '%vitriaEventLog_sdw%')
