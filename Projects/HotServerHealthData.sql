DROP VIEW	HotServerHealthData
GO
CREATE VIEW	HotServerHealthData
AS
WITH		LastCheckIn
			AS
			(
			...
			(
	-- 2005
						SELECT	'9.00.1399.00' [BuildNumber]
												, NULL [KBArticle]
	...
	-- 2008
			...
			UNION ALL	SELECT	'10.00.4000.00'	,'2285068'	,'http://support.microsoft.com/?kbid=2285068'																	,'SQL Server 2008 SP2 The second service pack for SQL Server 2008.'
			...
			UNION ALL	SELECT	'10.00.4279.00'	,'2498535'	,'http://support.microsoft.com/kb/2498535/en-us'																,'SQL Server 2008 SP2 CU 3 CU#3 for SQL Server 2008 Service Pack 2. Fixes include issues with Merge replication, mirroring suspension issues, various T-SQL issues among others.'
			UNION ALL	SELECT	'10.00.4285.00'	,'2527180'	,'http://support.microsoft.com/kb/2527180/en-us'																,'SQL Server 2008 SP2 CU 4 CU#4 for SQL Server 2008 Service Pack 2. Fixes include issues with Filestream, various T-SQL issues and some SSIS items among others.'
			UNION ALL	SELECT	'10.00.4316.00'	,'2582285'	,'support.microsoft.com/kb/2582285/en-us'																		,'SQL Server 2008 SP2 Cumulative Update 5'
			UNION ALL	SELECT	'10.00.4321.00'	,'2582285'	,NULL																											,'SQL Server 2008 SP2 Cumulative Update 6'
			UNION ALL	SELECT	'10.00.4323.00'	,'2617148'	,NULL																											,'SQL Server 2008 SP2 Cumulative Update 7'
			UNION ALL	SELECT	'10.00.5500.00'	,'2546951'	,NULL																											,'SQL Server 2008 Service Pack 3'
			UNION ALL	SELECT	'10.00.5766.00'	,'2617146'	,NULL																											,'Cumulative update package 1 for SQL Server 2008 Service Pack 3'
	...
	-- 2008 R2
			...
			UNION ALL	SELECT	'10.50.1807.0'	,'2591746'	,'http://support.microsoft.com/kb/2591746/en-us'																,'CU10 includes fixed for deadlocks, various query issues, CDC problems, and XML data used in replication.'
			...
			UNION ALL	SELECT	'10.50.2789.0'	,'2591748'	,'http://support.microsoft.com/kb/2591748/en-us'																,'CU#3 for SQL Server 2008 R2 SP1'
...
SELECT		HR.*
			,dbaadmin.dbo.Returnword(REPLACE(REPLACE(REPLACE(REPLACE
				(REPLACE(SI.SQLver,'- ',''),'(SP1) ',''),'Intel ','')
				,'(',''),')',''),1)										[SQLBuildNumber]
			,SBL.*
			,(SELECT MAX(BuildNumber) FROM SQLBuildList BN where PARSENAME(BN.BuildNumber,4)=PARSENAME(SBL.BuildNumber,4) AND PARSENAME(BN.BuildNumber,3)=PARSENAME(SBL.BuildNumber,3)) [MostRecentBuildNumber]
			,(SELECT COUNT(*)-1 FROM SQLBuildList SL WHERE BuildNumber between SBL.BuildNumber AND (SELECT MAX(BuildNumber) FROM SQLBuildList BN where PARSENAME(BN.BuildNumber,4)=PARSENAME(SBL.BuildNumber,4) AND PARSENAME(BN.BuildNumber,3)=PARSENAME(SBL.BuildNumber,3))) [UpdatesBehind]
...
LEFT JOIN	SQLBuildList SBL
		ON	SBL.BuildNumber = dbaadmin.dbo.Returnword(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(SI.SQLver,'- ',''),'(SP1) ',''),'Intel ',''),'(',''),')',''),1)
GO
...
GO
SELECT * FROM HotServerHealthData	

GO
SELECT	DISTINCT
		ServerName
		,SQLName
FROM [DBAcentral].[dbo].[DBA_ServerInfo] WHERE dbacentral.[dbo].[dbaudf_GetServerClass] ([SQLName]) = 'High' 
GO

SELECT (STUFF((SELECT ',' + ServerName FROM [DBAcentral].[dbo].[DBA_ServerInfo] WHERE dbacentral.[dbo].[dbaudf_GetServerClass] ([SQLName]) = 'High' ORDER BY 1
