...
;WITH		XMLNAMESPACES (DEFAULT 'http://schemas.microsoft.com/sqlserver/2004/07/showplan'),
			...
			AS
			(
            SELECT		DB_NAME(CAST(qpa.value AS INT)) DBName
			...
			CROSS APPLY	query_plan.nodes('//RelOp')					RelOp(col)

            WHERE		qpa.attribute	= 'dbid'
                    AND qpa.value		= DB_ID()
                    AND	RelOp.Col.value('(IndexScan/Object/@Index)[1]','sysname') = QUOTENAME(@IndexName,'[')
...
SELECT		TOP 100
			...
			,RelOp.Col.value('(IndexScan/Object/@Index)[1]','sysname') IndexName
...
WHERE		RelOp.Col.value('(IndexScan/Object/@Index)[1]','sysname') = QUOTENAME(@IndexName,'[')
