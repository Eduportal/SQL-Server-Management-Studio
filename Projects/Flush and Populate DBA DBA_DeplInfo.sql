...
FOR
SELECT		*
FROM		SEAFRESQLDBA01.dbaadmin.dbo.DBA_DeplInfo

DECLARE @name varchar(40)
OPEN NewItemCursor

FETCH NEXT FROM NewItemCursor INTO @DeplInfoId,@Domain,@Enviro_Type,@ServerName,@SQLName,@DBName,@Build_Number,@Build_Date,@Baseline_Date,@Record_Date
...
BEGIN
	...
	BEGIN
		...
		SET @Size = LEN(@Domain+@Enviro_Type+@ServerName+@SQLName+@DBName+@Build_Number+@Build_Date+@Baseline_Date+@Record_Date)+40
		...
		EXECUTE @RC = [WSS_Content_eCommOps].[dbo].[proc_AddListItem]
			...
			,@float1 = @DeplInfoId
			,@nvarchar1 = @Domain
			,@nvarchar3 = @Enviro_Type
			,@nvarchar4 = @ServerName
			,@nvarchar5 = @SQLName
			,@nvarchar6 = @DBName
			,@nvarchar7 = @Build_Number
			,@nvarchar8 = @Build_Date
			,@nvarchar9 = @Baseline_Date
			,@nvarchar10 = @Record_Date
