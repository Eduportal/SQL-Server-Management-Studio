/*
EXEC msdb.dbo.sp_set_sqlagent_properties @sqlserver_restart=1,@monitor_autostart=1
GO

exec xp_cmdshell 'SCHTASKS.EXE /CREATE /SC MINUTE /MO 5 /TN "RESTART SQL INSTANCE A" /ST 00:00:00 /SD 01/01/2000 /TR "NET START SQLAgent$A" /RU SYSTEM'
GO
SHUTDOWN --WITH NOWAIT
GO
exec xp_cmdshell 'SCHTASKS.EXE /DELETE /tn "RESTART SQL INSTANCE A" /F' 
GO
exec xp_fixeddrives
*/
USE [master]
...
DECLARE @TcpPort				VARCHAR(5)		,@AWEDefault			INT				,@save_servername		sysname			,@save_SQL_install_date	DateTime
		...
		,@ServerName_NEW		SYSNAME			,@MaxMemory				INT				,@charpos				INT				,@RedgateTested			bit
		...
		,@DefaultBackupDir		VarChar(8000)	,@RunSection			INT				,@SAPassword			sysname			,@RedgateConfigured		bit
		...
		,@SqbExecutionResultText VARCHAR(512)	,@DownloadDirectory		VARCHAR(260)	,@SqlProductVersion		NVARCHAR(20)	,@SqlMajorVersion		INT
...
IF (OBJECT_ID('tempdb..#SqbOutput')) IS NOT NULL	DROP TABLE #SqbOutput
...
SELECT	@RunSection			= 3		-- 1 = Pre-Config: Must Reboot After		5 = Fix Shares and enable & Run Checkin Job and fixup Redgate
									--		* Restart SQL							* check Service Packs and install redgate from 	
									-- 2 = Install Operations Databases			 	\\seafresqldba01\DBA_Docs\utilities\RedGate_SQLbackup\Red_Gate_6.4
									-- 3 = Clone Process						 	Key: 010-110-127441-4B91
									--		* Have Machine Renamed				6 = Final Redgate Config and Job Checkins
									-- 4 = Final Rename							7 = Run Summary Check
									--		* Restart SQL						
		...
		,@ServerToClone		= REPLACE(@machinename,'-N','')
		...
		,@DownloadDirectory = '\\seafresqldba01\DBA_Docs\utilities\RedGate_SQLbackup\Red_Gate_6.4'
		,@ExecRetryCount	= 10		,@SqbExecutionResult	= -1		,@SqbInstallRetry	= 1
...
SET		@DynamicCode	= 'SCHTASKS.EXE /CREATE /SC MINUTE /MO 1 /TN "RESTART SQL INSTANCE '+REPLACE(@DynamicCode,'$','')+'" /ST 00:00:00 /SD 01/01/2000 /TR "NET START SQLAgent'+@DynamicCode+'" /RU SYSTEM'
...
SET @RunSection = isnull(@RunSection,0)
...
-- SET AGENT SERVICE RESTART VALUES
EXEC msdb.dbo.sp_set_sqlagent_properties @sqlserver_restart=1,@monitor_autostart=1
PRINT	'Agent Service Restart Set'
...
SELECT 'Setting SQL Service Accounts Logins to ' +@ServiceActLogin
...
SELECT		@DynamicCode			= 'XCOPY \\'+@ServerString1+'\'+@ServerString2+'_dba_archive\*.* \\'+REPLACE(@@SERVERNAME,'\'+@@SERVICENAME,'')+'\'+REPLACE(@@SERVERNAME,'\','$')+'_dba_archive\ /Y'
PRINT		@DynamicCode
EXEC		XP_CMDSHELL  @DynamicCode
...
Section5:
	...
	-- WHERE dbaadmin.dbo.dbaudf_ReturnPart(REPLACE([Output],' ','|'),1) LIKE '%-N%'

	DECLARE @ShareName varchar(255)
	OPEN ShareCursor
	...
	BEGIN
		IF (@@fetch_status <> -2)
		BEGIN
			Print 'Dropping Share ' + @ShareName
...
SELECT 'Setting Redgate Service Account Logins to ' +@ServiceActLogin
...
SELECT 'Stoping Redgate Agent on Instance '+ @instancename
SET		@DynamicCode = 'NET STOP "SQL Backup Agent-'+ REPLACE(@instancename,'\','') +'"'
exec xp_cmdshell @DynamicCode

SELECT 'Starting Redgate Agent on Instance '+ @instancename
SET		@DynamicCode = 'NET START "SQL Backup Agent-'+ REPLACE(@instancename,'\','') +'"'
exec xp_cmdshell @DynamicCode

SELECT 'Fixing Redgate login on Instance '+ @instancename
