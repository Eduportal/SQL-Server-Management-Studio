USE [master]
GO
EXEC sp_configure 'show advanced option', 1;RECONFIGURE WITH OVERRIDE;EXEC sp_configure 'xp_cmdshell', 1;RECONFIGURE WITH OVERRIDE;	
GO
...
Section0:	-- Setup
BEGIN
	DECLARE @TcpPort				VARCHAR(5)		,@AWEDefault			INT				,@save_servername		sysname			,@save_SQL_install_date	DateTime
			...
			,@MaxMemory				INT				,@charpos				INT				,@RedgateTested			bit
			...
			,@SQLRestartRequired	BIT				,@ShareName				varchar(255)	,@ServerString1			sysname			,@ServerString2			sysname
			,@ServerString3			sysname			,@RunSection			INT				,@SectionDescript		VarChar(max)	,@Feature_Flop			bit
			,@Feature_NetSend		BIT				,@NetSendRecip			VarChar(1000)	,@Msg					VarChar(max)	,@MsgCommand			VarChar(8000)
			,@Feature_SnglStep		BIT				,@HealthCheckRecip		VarChar(1000)	,@Feature_SQLConfig		BIT				,@Feature_OpsDBs		BIT
			,@Feature_RedGate		BIT				,@Feature_Clone			BIT				,@Feature_HealthChk		BIT				,@Feature_Summary		BIT
			
SELECT		@Feature_SnglStep=0,@Feature_NetSend=0,@Feature_Flop=0,@Feature_Clone=0
			,@Feature_SQLConfig=1,@Feature_OpsDBs=1,@Feature_RedGate=0,@Feature_HealthChk=1,@Feature_Summary=1
...
BEGIN
	SET @Msg = '          Creating NEWServerDeployStep Extended Property';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sys.sp_addextendedproperty @Name = 'NEWServerDeployStep', @value = 1
END

	SET @Msg =	'          Creating Temp Tables';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	CREATE	TABLE	#ExecOutput			(TextOutput VARCHAR(MAX));
	...
	SET @Msg =	'          Getting xp_MSver Values';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SELECT	@instancename		= isnull('\'+nullif(REPLACE(@@SERVICENAME,'MSSQLSERVER',''),''),'')
			...
			,@ServerToClone		= CASE @Feature_Clone WHEN 0 THEN @@SERVERNAME ELSE COALESCE(@ServerToClone,REPLACE(@machinename,'-N',''),@@ServerName) END
	...
	SET @Msg =	'          Populating #EnvLogins Table';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'          Populating #ServerMatrix Table';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'          Cleaning #ServerMatrix Table';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	DELETE	#ServerMatrix
	WHERE	Port = '1433'
	...
	BEGIN -- CHECK FOR REDGATE
		SET @Msg =	'                Check Redgate';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		...
		SET @Msg =	'          Getting Redgate SQLBackup information';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		-- GET REDGATE INFORMATION
			...
			IF OBJECT_ID('master..sqbutility') IS NOT NULL
			  BEGIN
				...
				-- blank result sets)
				INSERT #ExecOutput EXECUTE master..sqbutility 30, @OldDllVersion OUTPUT;
				INSERT #ExecOutput EXECUTE master..sqbutility 1030, @OldExeVersion OUTPUT;
				INSERT #ExecOutput EXECUTE master..sqbutility 1021, @OldLicenseVersionId OUTPUT, NULL, @SerialNumber OUTPUT;

				-- Clean the temporary table
				DELETE FROM #ExecOutput;
	...
	SET @Msg =	'          Getting Port and Environment From #ServerMatrix';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT	@TcpPort			= Port
			,@EnvNum			= EnvNum
	...
	SET @Msg =	'          Getting Service Account Information';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'          Getting Data, Log, and Backup Locations';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'          Getting Current Instance Number';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'          Getting Current Authentication Mode';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'          Getting Current TCP/IP Port';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-- CHECK SERVER RENAME
	SET @Msg =	'          Check Server Name Status';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-- Get Install Date
	SET @Msg =	'          Get SQL Install Date';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	Select @save_SQL_install_date = (select createdate from master.sys.syslogins where name = 'BUILTIN\Administrators')
			
	--  Capture IP
	SET @Msg =	'          Get Current IP Address';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'            -- NSLOOKUP Method Found Current IP Address = ' + @save_ip ;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-- If @save_ip is null or @save_ip = '' or @save_ip = 'Error'
	   begin
		...
		SET @Msg =	'            -- PING Method Found Current IP Address = ' + @save_ip ;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	   end				
	...
	PRINT'          -----------------------------------------------------'
	PRINT'          -----------------------------------------------------'
	SET @Msg ='          -- DEPLOY TOOLS NEED FOR REST OF PROCCESS';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	PRINT'          -----------------------------------------------------'
	PRINT'          -----------------------------------------------------'
	...
	SET @Msg =	'          Creating Procedure [dbo].[dbasp_FileAccess_Write]';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'          Creating Function [dbo].[dbaudf_CPUInfo]';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET			@DynamicCode = 
	...
	SET @Msg =	'          Creating Function [dbo].[Dir]';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET	@DynamicCode		= 
	...
	SET @Msg =	'          Creating Procedure [dbo].[sp_configure_quiet]';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET	@DynamicCode		= 
...
SectionLoop:
	...
	PRINT	CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10)
			+'          -----------------------------------------------------'
	PRINT	'          -----------------------------------------------------'
	SET @Msg =	'          Starting Section ' + CAST(@RunSection AS VarChar) + ' : ' + @SectionDescript;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	PRINT	'          -----------------------------------------------------'
...
BEGIN
	...
	SET @Msg =	'              Creating Scheduled Task to Restart SQL if Service is Stopped';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'              Setting Recover to Simple on Model DB';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET	@DynamicCode	= 'ALTER DATABASE [model] SET RECOVERY SIMPLE WITH NO_WAIT'
	EXEC (@DynamicCode)

	-- SET SA Password
	SET @Msg =	'              Setting CHECK_EXPIRATION=OFF and CHECK_POLICY=OFF for [sa]';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'              Setting the [sa] Password';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET		@DynamicCode = 'USE MASTER;ALTER LOGIN [sa] WITH PASSWORD=N'''+@SAPassword+''''
	EXEC(@DynamicCode)

	SET @Msg =	'              Setting [sa] to Enabled';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	ALTER LOGIN [sa] ENABLE

	-- RENAME
	SET @Msg =	'              Checking if Server Rename has occured';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	IF isnull(nullif(@machinename,''),@@servername) != @@servername 
	BEGIN
		...
		SET @Msg = '                -- SERVER NAME CHANGED TO ' +  @machinename;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		SET @SQLRestartRequired = 1
	END
	ELSE SET @Msg = '                -- SERVER NAME ALREADY SET';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;

	-- CALCULATE AWE DEFAULT
	SET @Msg =	'              Calculating AWE Setting';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'              Calculating Max Memory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	END
	ELSE SET @Msg =	'                -- Calculation Overridden with fixed number';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;

	-- SET CONFIGURATION VALUES
	SET @Msg =	'              Setting sp_Configure Values';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET @Msg =	'                -- Max Memory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'max server memory (MB)', @MaxMemory;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- AWE';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'awe enabled', @AWEDefault;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- Show Advanced';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'show advanced option', 1;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- Agent XPs';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'Agent XPs', 1;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- XP_CmdShell';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'xp_cmdshell', 1;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- Ole Automation';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'Ole Automation Procedures', 1;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- CLR';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'clr enabled', 1;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- Trigger Results';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'disallow results from triggers', 1;
	RECONFIGURE WITH OVERRIDE; 
	SET @Msg =	'                -- Remote Proc Trans';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'remote proc trans', 1;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- Remote Admin';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC sp_configure_quiet 'remote admin connections', 1;
	RECONFIGURE WITH OVERRIDE;
	SET @Msg =	'                -- User Options';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	IF @LoginMode != 2
	BEGIN
		SET @Msg =	'              Changing Authentication Mode to Mixed';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		EXEC	master..xp_regwrite
					...
					,@type			= 'REG_DWORD' 
					,@value			= 2
		SET @Msg =	'Authentication Set to Mixed';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	END
	ELSE
	BEGIN
		SET @Msg =	'              Authentication Mode Already Set to Mixed';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	END

	-- CHANGE PORT
	IF		@TcpPort != isnull(nullif(@OldPort,''),'0')
	BEGIN
		SET @Msg =	'              Changing TCP/IP Port Number From ' + @OldPort + ' to ' + @TcpPort;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		SET		@RegKey				= 'SOFTWARE\Microsoft\Microsoft SQL Server\'+@InstanceNumber+'\MSSQLSERVER\SuperSocketNetLib\Tcp\IPAll\'
		EXEC	master..xp_regwrite
					@rootkey		= 'HKEY_LOCAL_MACHINE' 
					,@key			= @RegKey 
	...
	ELSE 
	BEGIN
		SET @Msg =	'              TCP/IP Port Number Already Set to ' + @TcpPort;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	END
	...
	BEGIN
		PRINT '*** DEFAULT DATA DRIVE DOES NOT EXIST, CAN NOT CONTINUE. ***'
		GOTO TheEnd
	END
		
	-- SET DEFAULT DATA, LOG, AND BACKUP DIRECTORYS
	SET @Msg =	'              Creating Default Data Directory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	IF NOT EXISTS (SELECT * FROM #File_Exists WHERE isDir = 1)
	BEGIN
		PRINT '*** DEFAULT DATA DIRECTORY DOES NOT EXIST, CAN NOT CONTINUE. ***'
		GOTO TheEnd
	END
	
	SET @Msg =	'              Setting Default Data Directory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	IF NOT EXISTS (SELECT * FROM #File_Exists WHERE isDir = 1)
	BEGIN
		PRINT '*** DEFAULT LOG DRIVE DOES NOT EXIST, CAN NOT CONTINUE. ***'
		GOTO TheEnd
	END
	
	SET @Msg =	'              Creating Default Log Directory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	IF NOT EXISTS (SELECT * FROM #File_Exists WHERE isDir = 1)
	BEGIN
		PRINT '*** DEFAULT LOG DIRECTORY DOES NOT EXIST, CAN NOT CONTINUE. ***'
		GOTO TheEnd
	END
	
	SET @Msg =	'              Setting Default Log Directory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC	master..xp_instance_regwrite
				@rootkey		= 'HKEY_LOCAL_MACHINE' 
				,@key			= 'Software\Microsoft\MSSQLServer\MSSQLServer' 
	...
	IF NOT EXISTS (SELECT * FROM #File_Exists WHERE isDir = 1)
	BEGIN
		PRINT '*** DEFAULT BACKUP DRIVE DOES NOT EXIST, CAN NOT CONTINUE. ***'
		GOTO TheEnd
	END
	
	SET @Msg =	'              Creating Default Backup Directory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	IF NOT EXISTS (SELECT * FROM #File_Exists WHERE isDir = 1)
	BEGIN
		PRINT '*** DEFAULT BACKUP DIRECTORY DOES NOT EXIST, CAN NOT CONTINUE. ***'
		GOTO TheEnd
	END
	
	SET @Msg =	'              Setting Default Backup Directory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC	master..xp_instance_regwrite
				@rootkey		= 'HKEY_LOCAL_MACHINE' 
				,@key			= 'Software\Microsoft\MSSQLServer\MSSQLServer' 
	...
	SET @Msg =	'              Setting SQL Agent History and Service Restart Values';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	BEGIN
		SET @Msg =	'              Relocating TempDB Data Device';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	ELSE 
	BEGIN
		SET @Msg =	'              TempDB Data Device is in the Correct Location';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	END

	IF NOT EXISTS (select * From TempDB..sysfiles where filename = @DefaultLogDir + '\templog.ldf')
	BEGIN
		SET @Msg =	'              Relocating TempDB Log Device';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	ELSE 
	BEGIN
		SET @Msg =	'              TempDB Log Device is in the Correct Location';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	END

	-- MODIFY NUMBER OF DATA DEVICES FOR TEMPDB
	SET @Msg =	'              Checking if Additional TempDB Data Devices are Needed';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	BEGIN
		...
		SET @Msg =	'                -- Adding Device tempdev_'+RIGHT('00'+CAST(@TempDBFiles AS VarChar),2);IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		SET @DynamicCode =
	...
	IF @Feature_RedGate = 1
	BEGIN
		SET @Msg =	'              Checking if RedGate Files are already Coppied';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		IF @RedgateInstalled = 1
		BEGIN
			SET @Msg =	'                  -- RedGate Files are already Coppied';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
			--goto SkipRedgateCopy
		END

		SET @Msg =	'              Copying RedGate SQLBackup';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
        ...
        INSERT #ExecOutput EXECUTE master..xp_cmdshell @SqbFileExistsExec; 
            
        -- Parse the output, pulling it from the temporary table (TOP 1 to get rid of subsequent rows)
        SET @Msg =	'                -- Checking For Network Files';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		...
		BEGIN
			SET @Msg =	'                -- Creating Directory';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;		
			SET @DynamicCode = 'MD "C:\Program Files (x86)\Red Gate\SQL Backup 6\"'
			EXEC	XP_CMDSHELL @DynamicCode, no_output
			
			SET @Msg =	'                -- Copying Files';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
			SET @DynamicCode = 'XCOPY "' + @RedGateNetworkPath + '\*.*" "C:\Program Files (x86)\Red Gate\SQL Backup 6\" /Q /C /Y /E'
			EXEC	XP_CMDSHELL @DynamicCode, no_output
			
			SET @Msg =	'                -- Creating Shortcut';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		...
		ELSE
		BEGIN
			SET @Msg =	'                -- Redgate Files were not found';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		END
...
SkipRedgateCopy:
	...
	-- INSTALL REDGATE BACKUP
		SET @Msg =	'              Checking if RedGate SQLBackup is already Installed';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		IF @OldDllVersion != 'Not Installed'
		BEGIN
			SET @Msg =	'                  -- Redgate SQLBackup is already Installed';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
			goto SkipRedgateInstall
		END

		SET @Msg =	'              Installing RedGate SQLBackup';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
        -- Check that the file exists (returning "1" if valid), if it doesn't, we cannot do the installation
        SET @SqbFileExistsExec = 'if exist "C:\Program Files (x86)\Red Gate\SQL Backup 6\SqbServerSetup.exe" echo 1';
        INSERT #ExecOutput EXECUTE master..xp_cmdshell @SqbFileExistsExec; 
            
        -- Parse the output, pulling it from the temporary table (TOP 1 to get rid of subsequent rows)
        SET @Msg =	'                -- Checking For Install File';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
        SELECT TOP 1 @SqbExistsResult = CAST(TextOutput AS VARCHAR(50)) FROM #ExecOutput; 
        ...
        SET @Msg =	'                -- Checking if exitcode file can be written';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
        ...
        INSERT #ExecOutput EXECUTE master..xp_cmdshell @SqbTestFileExistsExec; 
		...
		BEGIN
            ...
            SET @Msg =	'                -- Building Command Line';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
            ...
            WHILE @ExecRetryCount > 0 AND @SqbInstallRetry = 1
              BEGIN
                ...
                SET @Msg =	'                -- Installing...';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
                EXECUTE master..xp_cmdshell @SqbSetupExec, no_output;
                
                SET @Msg =	'                -- Reading Exit Code File';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
                INSERT #ExecOutput EXECUTE master..xp_cmdshell @TypeExitCodeFileExec; 
                -- Parse the output, pulling it from the temporary table
                ...
                IF @SqbExecutionResult = 5
                  BEGIN
					SET @Msg =	'                  -- Failed, Waiting 10 Seconds to Retry';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
            ...
            SET @Msg =	'                -- Deleting Exit Code File';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
            INSERT #ExecOutput EXECUTE master..xp_cmdshell @DelExitCodeFileExec;

            -- Parse the output code, and generate the necessary text
            IF @SqbExecutionResult = 0
            BEGIN
				SET @Msg =	'                  -- Successfull';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
        ...
        ELSE
          ...
          END
		SET @Msg =	'                  -- ' + @SqbExecutionResultText;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;

		
         -- If the SQL Backup components are now installed, attempt to get the current version details.
			SET @Msg =	'                -- ReColecting RedGate SQLBackup Values';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
            ...
            IF OBJECT_ID('master..sqbutility') IS NOT NULL
              BEGIN
                -- A version has been installed, we need to find out which (we use #ExecOutput to get rid of the
                -- blank result sets)
                INSERT #ExecOutput EXECUTE master..sqbutility 30, @NewDllVersion OUTPUT;
                INSERT #ExecOutput EXECUTE master..sqbutility 1030, @NewExeVersion OUTPUT;
                INSERT #ExecOutput EXECUTE master..sqbutility 1021, @NewLicenseVersionId OUTPUT, NULL, @SerialNumber OUTPUT;
...
SkipRedgateInstall:       
	...
	For Each objservice in colServiceList
		If objService.name = "MSSQL'+@ServiceExt+'" or objService.name = "SQLAgent'+@ServiceExt+'" or objService.name = "SQL Backup Agent-'+@instancename+'" Then
...
BEGIN
	...
	-----------------------------------------------------
	-----------------------------------------------------
	SET @Msg =	'                DROP AND CREATE EMPTY DBAADMIN DATABASE';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET	@DBName	= 'dbaadmin'
	SET	@DynamicCode	= 
	...
	-- SET Model DB Recovery to Simple
	SET @Msg =	'              Setting Recover to Simple on dbaadmin DB';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-----------------------------------------------------
	-----------------------------------------------------
	SET @Msg =	'                DROP AND CREATE EMPTY DBAPERF DATABASE';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET	@DBName	= 'dbaperf'
	SET	@DynamicCode	= 
	'IF EXISTS (SELECT name FROM sys.databases WHERE name = N''' + @DBName +''')
		ALTER DATABASE [' + @DBName +'] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE
	...
	-- SET Model DB Recovery to Simple
	SET @Msg =	'              Setting Recover to Simple on dbaperf DB';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-----------------------------------------------------
	-----------------------------------------------------
	SET @Msg =	'                DROP AND CREATE EMPTY DEPLINFO DATABASE';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET	@DBName	= 'deplinfo'
	SET	@DynamicCode	= 
	'IF EXISTS (SELECT name FROM sys.databases WHERE name = N''' + @DBName +''')
		ALTER DATABASE [' + @DBName +'] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE
	...
	-- SET Model DB Recovery to Simple
	SET @Msg =	'              Setting Recover to Simple on deplinfo DB';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-----------------------------------------------------
	-----------------------------------------------------
	SET @Msg =	'                START DEPLOY DBAADMIN DATABASE';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;

	-- COPY FILES
	SET @Msg =	'                Copy "System32" files';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	Select	@cmd = 'xcopy \\' + @central_server + '\' + @central_server + '_builds\dbaadmin\system32\*.*  %windir%\system32 /Q /C /Y'
	SET @Msg =		'                  -- ' + @cmd;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	exec	master.sys.xp_cmdshell @cmd, no_output

	SET @Msg =	'                Copy C:\DBA_DiskCheck_DoNotDelete.txt';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-- DEPLOY DB
	SET @Msg =		'                Identify Most Recent DBAADMIN Build';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT		TOP 1 
	...
	SET @Msg =		'                Deploy DBAADMIN Database';IF @Feature_NetSend=1 BEGIN SET @Msg	= 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @Msg, no_output; END Print @Msg;
	SET @Msg =		'                  -- ' + @DynamicCode;IF @Feature_NetSend=1 BEGIN SET @Msg	= 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @Msg, no_output; END Print @Msg;
	EXEC		XP_CMDSHELL  @DynamicCode, no_output 

	-- BUILD SHARES
	SET @Msg =	'                Build Shares';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	
	TRUNCATE TABLE #ExecOutput
	SET @DynamicCode	= 'sqlcmd -S' + @@ServerName + ' -E -Q"EXEC dbaadmin.dbo.dbasp_dba_sqlsetup ''' + @DefaultBackupDir + '''"'
	INSERT INTO #ExecOutput
	...
	SET @Msg = ''
	SELECT @Msg=@Msg+TextOutput+CHAR(13)+CHAR(10) FROM #ExecOutput;Print SUBSTRING(@Msg,1,CHARINDEX(@Msg,CHAR(10),7000)) ;Print SUBSTRING(@Msg,CHARINDEX(@Msg,CHAR(10),7000)+1,CHARINDEX(@Msg,CHAR(10),CHARINDEX(@Msg,CHAR(10),7000)+7000)) ;

	-- FIX JOB OUTPUTS
	SET @Msg =	'                Fix Job Outputs';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC dbaadmin.dbo.dbasp_FixJobOutput

	-- DISABLE JOBS
	SET @Msg =	'                Disable Check-In and Archive Jobs';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-----------------------------------------------------
	-----------------------------------------------------
	SET @Msg =	'                START DEPLOY DBAPERF DATABASE';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET @Msg =		'                Identify Most Recent DBAPERF Build';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT		TOP 1 
				@DynamicCode = 'SQLCMD -S '+ @@SERVERNAME + ' -E -i "' + path +'"'
	FROM		master.dbo.dir('\\seafresqldba01\builds\dbaperf\production\')
	WHERE		Name Like 'dbaperf_2005_release%'
	ORDER BY	ModifyDate DESC
	SET @Msg =		'                Deploy DBAPERF Database';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET @Msg =		'                  -- ' + @DynamicCode;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC		XP_CMDSHELL  @DynamicCode, no_output
	...
	-----------------------------------------------------
	-----------------------------------------------------
	SET @Msg =	'                START DEPLOY DEPLINFO DATABASE';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET @Msg =		'                Identify Most Recent DEPLINFO Build';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT		TOP 1 
				@DynamicCode = 'SQLCMD -S '+ @@SERVERNAME + ' -E -i "' + path +'"'
	FROM		master.dbo.dir('\\seafresqldba01\builds\deplinfo\')
	WHERE		Name Like 'deplinfo_2005_20%'
	ORDER BY	ModifyDate DESC
	SET @Msg =		'                Deploy DEPLINFO Database';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET @Msg =		'                  -- ' + @DynamicCode;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC		XP_CMDSHELL  @DynamicCode, no_output

	-- CREATE JOBS
	SET @Msg =		'                Create Deployment Jobs';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	UPDATE #ExecOutput SET TextOutput = '                  -- ' + TextOutput
	SET @Msg = ''
	SELECT @Msg=@Msg+TextOutput+CHAR(13)+CHAR(10) FROM #ExecOutput;Print @Msg;
...
BEGIN
	...
	SET @Msg =		'                Copying dba_archive directory from ' + @ServerString1;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT		@DynamicCode			= 'XCOPY \\'+@ServerString1+'\'+@ServerString2+'_dba_archive\*.* \\'+REPLACE(@@SERVERNAME,'\'+@@SERVICENAME,'')+'\'+REPLACE(@@SERVERNAME,'\','$')+'_dba_archive\ /Q /C /Y'
	SET @Msg =		'                  -- ' + @DynamicCode;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	EXEC		XP_CMDSHELL  @DynamicCode, no_output 
	...
	SET @Msg =	'                Creating User Databases';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-- CHANGE DB OWNER
	SET @Msg =	'                Running _SYSchgdbowner.gsql';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	-- ADD MASTER LOGINS
	SET @Msg =	'                Running _SYSaddmasterlogins.gsql';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT		@ScriptPath			= '\\'+@ServerString1+'\'+@ServerString2+'_dba_archive\'+@ServerString3+'_SYSaddmasterlogins.gsql'
				,@DynamicCode = 'SQLCMD -S '+ @@SERVERNAME + ' -E -i "' + @ScriptPath +'"'
	EXEC		XP_CMDSHELL  @DynamicCode, no_output 			

	-- CREATE DB USERS
	SET @Msg =	'                Running _SYScreateDBusers.gsql';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT		@ScriptPath			= '\\'+@ServerString1+'\'+@ServerString2+'_dba_archive\'+@ServerString3+'_SYScreateDBusers.gsql'
				,@DynamicCode = 'SQLCMD -S '+ @@SERVERNAME + ' -E -i "' + @ScriptPath +'"'
	EXEC		XP_CMDSHELL  @DynamicCode, no_output 			

	-- ADD DB ROLES
	SET @Msg =	'                Running _SYSadddbroles.gsql';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT		@ScriptPath			= '\\'+@ServerString1+'\'+@ServerString2+'_dba_archive\'+@ServerString3+'_SYSadddbroles.gsql'
				,@DynamicCode = 'SQLCMD -S '+ @@SERVERNAME + ' -E -i "' + @ScriptPath +'"'
	EXEC		XP_CMDSHELL  @DynamicCode, no_output 			

	-- ADD DB ROLE MEMBERS
	SET @Msg =	'                Running _SYSadddbrolemembers.gsql';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'                Running _SYSaddsrvrolemembers.gsql';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SELECT		@ScriptPath			= '\\'+@ServerString1+'\'+@ServerString2+'_dba_archive\'+@ServerString3+'_SYSaddsrvrolemembers.gsql'
				,@DynamicCode = 'SQLCMD -S '+ @@SERVERNAME + ' -E -i "' + @ScriptPath +'"'
	EXEC		XP_CMDSHELL  @DynamicCode, no_output 			

	-- ADD SYSTEM MESSAGES
	SET @Msg =	'                Running _SYSaddsysmessages.gsql';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
...
BEGIN
	...
	BEGIN
		...
		SET @SQLRestartRequired = 1
	END
	ELSE SET @Msg =	'WAITING FOR SQL RENAME BY WEB TEAM';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
...
BEGIN
	...
	SET @Msg =	'                Drop Existing Shares';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	BEGIN
		IF (@@fetch_status <> -2)
		BEGIN
			SET @Msg =	'                  -- Dropping Share ' + @ShareName;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
...
BEGIN
	...
	SET @Msg =	'                Changing DB Owner in all User Databases to [sa]';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;	
	exec sp_MSForEachDB 'IF DB_ID(''?'') > 4 ALTER AUTHORIZATION ON Database::? TO [sa]'  

	-- SET SERVICE ACCOUNT ON Redgate Backup Service
	IF @Feature_RedGate = 1
	BEGIN
		SET @Msg =	'                Setting Redgate Service Account Logins to ' +@ServiceActLogin;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		SET		@DynamicCode = 'strComputer = "."
		Set objWMIService = GetObject("winmgmts:" _
			& "{impersonationLevel=impersonate}!\\" & strComputer & "\root\cimv2")
		Set colServiceList = objWMIService.ExecQuery _
		...
		SET @Msg =	'                Stoping Redgate Agent on Instance '+ @instancename;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		SET		@DynamicCode = 'NET STOP "SQL Backup Agent-'+ REPLACE(@instancename,'\','') +'"'
		exec xp_cmdshell @DynamicCode, no_output

		SET @Msg =	'                Starting Redgate Agent on Instance '+ @instancename;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		SET		@DynamicCode = 'NET START "SQL Backup Agent-'+ REPLACE(@instancename,'\','') +'"'
		exec xp_cmdshell @DynamicCode, no_output

		--SET @Msg =	'                Fixing Redgate login on Instance '+ @instancename;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'                Clear Out Local DBA Info Tables';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
...
BEGIN
	...
	SET @Msg =	'                Delete Scheduled Task to Restart SQL Services';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	SET		@DynamicCode	= isnull('$'+nullif(REPLACE(@@SERVICENAME,'MSSQLSERVER',''),''),'')
	SET		@DynamicCode	= 'SCHTASKS.EXE /DELETE /TN "RESTART SQL INSTANCE '+REPLACE(@DynamicCode,'$','')+'" /F'
	EXEC	XP_CMDSHELL @DynamicCode, no_output
	
	-- ENABLE CHECKIN JOB
	SET @Msg =	'                Enable Check-In and Archive Jobs';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'                Run Job : ' + @job_name;IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'                Run Job : UTIL - DBA Archive process';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	...
	SET @Msg =	'                Flush DNS';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
	exec xp_cmdshell 'ipconfig /flushdns', no_output 
	
	-- RUN HEALTH CHECK
	If @Feature_HealthChk = 1
	BEGIN
		SET @Msg =	'                Run Health Check Report';IF @Feature_NetSend=1 BEGIN SET @MsgCommand = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @MsgCommand, no_output; END Print @Msg;
		TRUNCATE TABLE #ExecOutput
...
BEGIN
	...
	SELECT		@@ServerName SQLName,* From dbo.dir(@DynamicCode)
...
BEGIN
	...
	IF @SQLRestartRequired = 1
	BEGIN
		SET @Msg = 'WAITING FOR SQL RESTART';IF @Feature_NetSend=1 BEGIN SET @DynamicCode = 'NET SEND ' + @NetSendRecip + ' ' + @Msg; exec xp_CmdShell @DynamicCode, no_output; END Print @Msg;
	END	
END
TheEnd:		-- The End
BEGIN
...
--	EXEC sys.sp_addextendedproperty @Name = 'NEWServerDeployStep', @value = 1
--ELSE
--	EXEC sys.sp_updateextendedproperty @Name = 'NEWServerDeployStep', @value = 1
--GO
--DECLARE	@DynamicCode	VarChar(8000)
---- CREATE SCHEDULED TASK TO RESTART SQL EVERY MINUTE
--PRINT	'              Creating Scheduled Task to Restart SQL if Service is Stopped'
--SET		@DynamicCode	= isnull('$'+nullif(REPLACE(@@SERVICENAME,'MSSQLSERVER',''),''),'')
--SET		@DynamicCode	= 'SCHTASKS.EXE /CREATE /SC MINUTE /MO 1 /TN "RESTART SQL INSTANCE '+REPLACE(@DynamicCode,'$','')+'" /ST 00:00:00 /SD 01/01/2000 /TR "NET START SQLAgent'+@DynamicCode+'" /RU SYSTEM /F'
--EXEC	XP_CMDSHELL @DynamicCode, no_output
--GO
