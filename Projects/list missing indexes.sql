DECLARE @PopulateDMVsForAll INT
...
;WITH	XMLNAMESPACES (DEFAULT 'http://schemas.microsoft.com/sqlserver/2004/07/showplan')
		...
		SELECT	ecp.plan_handle
				...
				, CAST(MissingIndex.query ('.') AS XML)					AS Statements 
		...
		WHERE		(
				MissingIndex.exist ('.//MissingIndex[@Database = sql:variable("@database_name")]') = 1
				OR @database_name IS NULL
				OR @PopulateDMVsForAll = 1
				)
			AND	(
				MissingIndex.exist ('.//MissingIndex[@Table = sql:variable("@table_name")]') = 1
		...
		SELECT		@@ServerName								AS server_name
				...
				, REPLACE(CAST(Mi.query
					...
					where $cg/@Usage="EQUALITY" 
					or $cg/@Usage="INEQUALITY"
					return $cg/Column/@Name	)')
					AS NVarchar(MAX)),'] [','], [')					AS IndexColumns
				, REPLACE(CAST(Mi.query
					('data( for $cg in .//ColumnGroup
					where $cg/@Usage="INCLUDE"
					return $cg/Column/@Name	)')
					AS NVarchar(MAX)),'] [','], [')					AS IncludeColumns
				,REPLACE(REPLACE(REPLACE(CAST(Mi.query
					('data( for $cg in .//ColumnGroup
					where $cg/@Usage="EQUALITY" 
					or $cg/@Usage="INEQUALITY"
					return $cg/Column/@ColumnId )')
					AS NVarchar(MAX)),'[',''),']',''),' ','_')			AS IndexColumnIDs
				,REPLACE(REPLACE(REPLACE(CAST(Mi.query
					('data( for $cg in .//ColumnGroup
					where $cg/@Usage="INCLUDE"
					return $cg/Column/@ColumnId )')
					AS NVarchar(MAX)),'[',''),']',''),' ','_')			AS IncludeColumnIDs
					
		From		PlanData								AS pd
		CROSS APPLY	Statements.nodes ('.//MissingIndex')					AS St(Mi)
		)
