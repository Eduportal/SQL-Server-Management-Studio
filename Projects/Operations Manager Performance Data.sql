...
;WITH		GroupMembersCTE				-- BASE GROUP OF COMPUTERS TO PULL 
			...
			,PerformanceRanges		-- DEFINE RANGES
			AS
			(
			SELECT		[FQDN]
						,[ObjectName]
						...
						,MIN([Value])										[Min_Value]
						,MIN([Value]) + ((MAX([Value]) - MIN([Value]))/4)	[Lo_Value]
						,MIN([Value]) + ((MAX([Value]) - MIN([Value]))/2)	[Mid_Value]
						,MAX([Value]) - ((MAX([Value]) - MIN([Value]))/4)	[Hi_Value]
						,MAX([Value])										[Max_Value]
