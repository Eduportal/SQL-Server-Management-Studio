...
GO
IF OBJECT_ID('dbasp_Logship_Fix') IS NOT NULL
	DROP PROCEDURE [dbo].[dbasp_Logship_Fix]
GO
...
AS
	...
	;WITH		SourceFiles
			...
			AS
			(
			SELECT		MAX(CASE RIGHT(name,3) WHEN 'SQB' THEN [ModifyDate] END)	MostRecent_Full
					,MAX(CASE RIGHT(name,3) WHEN 'SQD' THEN [ModifyDate] END)	MostRecent_Diff
					,MAX(CASE RIGHT(name,3) WHEN 'SQT' THEN [ModifyDate] END)	MostRecent_Log
			FROM		SourceFiles
			)
			,NeededFiles
			AS
			...
			WHERE		RIGHT(S.Name,3) = 'SQB' AND S.ModifyDate = (SELECT top 1 MostRecent_Full FROM MostRecent)
				OR	(
					RIGHT(S.Name,3) = 'SQD' 
					AND S.ModifyDate = (SELECT top 1 MostRecent_Diff FROM MostRecent) 
					AND S.ModifyDate > (SELECT top 1 MostRecent_Full FROM MostRecent)
					)
				OR	(
					RIGHT(S.Name,3) = 'SQT' AND S.ModifyDate > (SELECT CASE WHEN MostRecent_Full > MostRecent_Diff THEN MostRecent_Full ELSE MostRecent_Diff END FROM MostRecent)
					)		
			)
			,QueuedFiles
			AS
			...
			SELECT		@MostRecent_Full	LastAppliedFul
					...
					,N.name	
					,CASE	WHEN Q.name IS NULL AND P.Name Is Not NULL	THEN P.[Path]
						WHEN A.name IS NOT NULL				THEN A.[Path]
						WHEN Q.name IS NULL AND P.Name IS NULL		THEN N.[Path]
						WHEN Q.name IS NOT NULL AND P.Name IS NULL	THEN Q.[Path]
						END AS [Path]	
					,N.ModifyDate	
					,N.IsFileSystem	
					,N.IsFolder	
	...
	BEGIN
		...
		BEGIN
			...
			EXEC dbaadmin.dbo.dbasp_UnlockAndDelete @FileName,1,1,0
		END
		FETCH NEXT FROM PurgeOldFiles INTO @FileName
	END
	CLOSE PurgeOldFiles
