...
FROM		[SEAPSQLDBA01].[dbacentral].[INFORMATION_SCHEMA].[COLUMNS] 
...
SET		@Columns3 = ''
SELECT		@Counter_nvarchar	= CASE WHEN [Data_type] NOT IN ('datetime','smalldatetime'
									,'ntext','text','float','bigint'
									,'smallint','tinyint','int'
									,'numeric','decimal','bit')	THEN @Counter_nvarchar	+ CASE @Counter_nvarchar WHEN 1 THEN 2 ELSE 1 END ELSE @Counter_nvarchar END
		,@Counter_datetime	= CASE WHEN [Data_type] IN ('datetime','smalldatetime')		THEN @Counter_datetime	+ 1 ELSE @Counter_datetime END
		,@Counter_ntext		= CASE WHEN [Data_type] IN ('ntext','text')			THEN @Counter_ntext	+ 1 ELSE @Counter_ntext END
		,@Counter_float		= CASE WHEN [Data_type] IN ('float','numeric','decimal','bigint','smallint','tinyint','int')	THEN @Counter_float	+ 1 ELSE @Counter_float END
		,@Counter_int		= CASE WHEN [Data_type] IN ('smallint','tinyint','int')		THEN @Counter_int	+ 1 ELSE @Counter_int END
		,@Counter_bit		= CASE WHEN [Data_type] IN ('bit')				THEN @Counter_bit	+ 1 ELSE @Counter_bit END
		,@Columns1		= COALESCE(@Columns1+',@'+[column_name],'@'+[column_name])
		,@Columns2		= COALESCE(@Columns2+'+@'+[column_name],'@'+[column_name])
		,@Columns3		= @Columns3 +'			,@'
						+ CASE [Data_type]
							WHEN 'datetime'		THEN 'datetime' + CAST(@Counter_datetime AS VARCHAR(10))
							WHEN 'smalldatetime'	THEN 'datetime' + CAST(@Counter_datetime AS VARCHAR(10))
							WHEN 'ntext'		THEN 'ntext' + CAST(@Counter_ntext AS VARCHAR(10))
							WHEN 'text'		THEN 'ntext' + CAST(@Counter_ntext AS VARCHAR(10))
							WHEN 'float'		THEN 'float' + CAST(@Counter_float AS VARCHAR(10))
							WHEN 'numeric'		THEN 'float' + CAST(@Counter_float AS VARCHAR(10))
							WHEN 'decimal'		THEN 'float' + CAST(@Counter_float AS VARCHAR(10))
							WHEN 'bigint'		THEN 'float' + CAST(@Counter_float AS VARCHAR(10))
							WHEN 'smallint'		THEN 'float' + CAST(@Counter_float AS VARCHAR(10))
							WHEN 'tinyint'		THEN 'float' + CAST(@Counter_float AS VARCHAR(10))
							WHEN 'int'		THEN 'float' + CAST(@Counter_float AS VARCHAR(10))
							WHEN 'bit'		THEN 'bit' + CAST(@Counter_bit AS VARCHAR(10))
							ELSE 'nvarchar' + CAST(@Counter_nvarchar AS VARCHAR(10)) END
						+' = @'+[column_name]+CHAR(13)+CHAR(10)
		,@Columns4		= COALESCE	(
							@Columns4+'	,@'+[column_name]+' '
							+ CASE [Data_type] 
								WHEN 'binary'		THEN [Data_type]+QUOTENAME(CHARACTER_MAXIMUM_LENGTH,'(')
								WHEN 'char'		THEN [Data_type]+QUOTENAME(CHARACTER_MAXIMUM_LENGTH,'(')
								WHEN 'decimal'		THEN [Data_type]+QUOTENAME(NUMERIC_PRECISION+','+NUMERIC_SCALE,'(') 
								WHEN 'nchar'		THEN [Data_type]+QUOTENAME(CHARACTER_MAXIMUM_LENGTH,'(')
								WHEN 'numeric'		THEN [Data_type]+QUOTENAME(NUMERIC_PRECISION+','+NUMERIC_SCALE,'(') 
								WHEN 'nvarchar'		THEN [Data_type]+QUOTENAME(CHARACTER_MAXIMUM_LENGTH,'(')
								WHEN 'varbinary'	THEN [Data_type]+QUOTENAME(CHARACTER_MAXIMUM_LENGTH,'(')
								WHEN 'varchar'		THEN [Data_type]+QUOTENAME(CHARACTER_MAXIMUM_LENGTH,'(')
								ELSE [Data_type] END
							,'	@'+[column_name]+' '
							+ CASE [Data_type] 
								WHEN 'binary'		THEN [Data_type]+QUOTENAME(CHARACTER_MAXIMUM_LENGTH,'(')
...
DECLARE	@SQLServerID int
	...
	,@dbaperf_Version nvarchar(128)
	,@deplinfo_Version nvarchar(128)
	...
	,@IndxSnapshot_interval SYSNAME
	,@CLR_state SYSNAME
	,@FrameWork_ver SYSNAME
	,@FrameWork_dir SYSNAME
	,@PowerShell CHAR(1)
	...
	,@boot_pae char(1)
	,@boot_userva char(1)
	,@Pagefile_maxsize SYSNAME
	,@Pagefile_available SYSNAME
	,@Pagefile_inuse SYSNAME
...
FOR
SELECT		SQLServerID,ServerName,ServerType,SQLName,SQLEnv,Active,Filescan,SQLmail,modDate,SQLver,SQLinstallDate,SQLinstallBy,SQLrecycleDate,SQLSvcAcct,SQLAgentAcct,SQLStartupParms,SQLScanforStartupSprocs,dbaadmin_Version,dbaperf_Version,DEPLinfo_Version,backup_type,LiteSpeed,RedGate,awe_enabled,MAXdop_value,Memory,SQLmax_memory,tempdb_filecount,FullTextCat,Assemblies,Filestream_AcsLvl,Mirroring,Repl_Flag,LogShipping,LinkedServers,ReportingSvcs,LocalPasswords,DEPLstatus,IndxSnapshot_process,IndxSnapshot_inverval,CLR_state,FrameWork_ver,FrameWork_dir,PowerShell,OracleClient,TNSnamesPath,DomainName,iscluster,SAN,Port,Location,IPnum,CPUphysical,CPUcore,CPUlogical,CPUtype,OSname,OSver,OSinstallDate,OSuptime,MDACver,IEver,AntiVirus_type,AntiVirus_Excludes,boot_3gb,boot_pae,boot_userva,Pagefile_maxsize,Pagefile_available,Pagefile_inuse,Pagefile_path,TimeZone,SystemModel,MOMverifyDate
FROM		SEAPSQLDBA01.dbacentral.dbo.DBA_ServerInfo
