...
*/
set @RowModCounterThreshold=1000 --change if 50K rows have changed
	...
	,Description=
	  ...
	  +case when src.IndexRowModCounter / (src.EstimatedTableRowcount * src.IndexPercentageOfTable) > @RowModPercentageThreshold
		then N'Percentage changed ('
			+convert(nvarchar(9),convert(float,src.AdjustedPercentageChanged*100.0))
			+N'%) is above threshold ('
			+convert(nvarchar(9),convert(float,@RowModPercentageThreshold*100.0))
			+N'%) '
		else N'' end
