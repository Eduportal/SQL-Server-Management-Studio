...
WHERE [name] = DB_NAME()
--(Status & 512) = 0 /*NOT IN (536,528,540,2584,1536,512,4194841)*/ 
ORDER BY [Name]
...
' SpaceUsed_MB	= NULL,'+
' MaxSize_MB	= CASE [maxsize] WHEN -1 THEN -1 WHEN 0 THEN Cast([size] AS Bigint)*8/1024.00 ELSE Cast([maxsize] AS Bigint)*8/1024.00 END,'+
' NextExtent_MB	= CASE WHEN (1048576&[status])=1048576 THEN (Cast([growth] AS Bigint)/100.00)*(Cast([size] AS Bigint)*8/1024.00) WHEN [growth]=0 THEN 0 ELSE Cast([growth] AS Bigint)*8/1024.00 END,'+
