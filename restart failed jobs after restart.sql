...
DECLARE RestartJobCursor CURSOR
FOR
-- SELECT QUERY FOR CURSOR
SELECT        O.name [JobName]
              ,(SELECT step_name FROM MSDB.DBO.SYSJOBSTEPS WHERE job_id = O.job_id and step_id = COALESCE(OA.last_executed_step_id,1)) [StepName]
...
WHERE        JH.RUN_STATUS != 1		-- LAST RUN WAS NOT SUCESSFULL
	AND	CASE			-- NOT CURRENTLY RUNNING
                   WHEN OA0.run_requested_date IS NULL THEN 'Idle'
                   WHEN JH0.RUN_STATUS = 0 THEN 'Failed'
                   WHEN JH0.RUN_STATUS = 1 THEN 'Succeeded'
                   WHEN JH0.RUN_STATUS = 2 THEN 'Retry'
		   ...
		   WHEN JH0.RUN_STATUS = 4 THEN 'Running'
                   WHEN JH0.RUN_STATUS IS NULL AND OA0.start_execution_date IS NOT NULL AND OA0.stop_execution_date IS NULL THEN 'Running'
                   ELSE 'Unknown'
                   END NOT IN ('Running','Succeeded')

OPEN RestartJobCursor;
FETCH RestartJobCursor INTO @JobName,@StepName;
...
BEGIN
	...
	END CATCH
              ...
              ----------------------------
       END
      FETCH NEXT FROM RestartJobCursor INTO @JobName,@StepName;
END
CLOSE RestartJobCursor;
DEALLOCATE RestartJobCursor;
