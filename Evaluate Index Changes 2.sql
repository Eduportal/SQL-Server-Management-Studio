...
BEGIN
	...
	WHERE		s.user_scans + s.user_seeks + s.user_lookups = 0 -- INDEX IS NEVER USED
		OR	CONVERT	(
				...
				) >= 1000 -- INDEX IS UPDATED MUCH MORE THAN IT IS USED
	ORDER BY	6 desc -- [Current_IndexSizekb] DESC		
	option (recompile);
...
SELECT		'  DROP INDEX ' + QUOTENAME(i.name) + ' ON ' + QUOTENAME(OBJECT_SCHEMA_NAME(object_id)) + '.' + QUOTENAME(object_name(object_id)) 
FROM		sys.indexes I 
WHERE		i.name like 'AMIX_%'
