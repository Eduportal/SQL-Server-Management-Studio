...
SELECT        @OldLogin            = 'AMER\SG_AMER_Cognos_Dev'
              ,@NewLogin           = 'EMEA\nabidakun'
...
IF EXISTS (SELECT * from sys.database_principals where Type IN (''U'',''G'') and name = '''+@OldLogin+''')
BEGIN
...
END'
   ...
   exec sp_MsForEachDB 'PRINT''-- Checking ? ...''','?',@CMD
